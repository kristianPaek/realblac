...
*
*       @author:            eMeeting Ltd
*       @copyright:         eMeeting Ltd
*       @version:           Revision: 9.0
*       @date:              22nd April 2007
*       @last modified:     29th April 2007
*       @license            Licensed Software
*
*       @website:           www.datingscripts.co.uk
*       @email:             info@datingscripts.co.uk
*
*       @requirements:      Valid Software License Key
*
*       =========================================================================
*
*       Copyright (C) 2009  eMeeting Ltd
*       
...
*       with this program; if not, write to eMeeting Ltd or view the website for more details
*
*       =========================================================================
*/
...
///////////////////////////////////////////////////////////////////////////////////////////
define('subd','../');
define('theme_download','http://www.datingscripts.co.uk');
define('powered_link','http://www.datingscripts.co.uk');
define('sms_link','http://www.datingscripts.co.uk/order/validate_sms_credits.php?l=');
define('upgrade_link','http://www.datingscripts.co.uk/order/validate_upgrade.php');
define('upgrade_link_buy','http://datingscripts.co.uk');
define('icon_edit','<img src="inc/images/icons/edit.gif" align="absmiddle">');
...
"tb_captions" => array("ID","Title","Date"),
		...
		case "articles": { //SELECT  WHERE articles.cat_id = articles_cat.id ORDER BY articles.id DESC
			...
			$GLOBALS['SEARCH_DATA'] = array( //forum_posts, forum_forums, forum_topics
			"tb_system" => "articles",  
			"tb_captions" => array("ID","Title","Views","Category ID","Category"),
			"tb_data" => array("articles.id","articles.title","articles.views","articles.cat_id","articles_cat.name AS Category"),
			"tb_data_name" => array("articles.","articles_cat."), // used to display the table data			 
			"tb_tables" => "articles INNER JOIN articles_cat ON ( articles.cat_id = articles_cat.id) ",
			"tb_where" => "", //WHERE files.album_id = albums.id
		...
		case "files": { //SELECT , files.* FROM  WHERE members.id=files.uid AND files.approved='yes' ORDER BY files.id ASC LIMIT 0, 12 
			...
			if(isset($_GET['startvalue']) && $_GET['startvalue'] !="" && $_GET['startvalue'] !="0"){
				...
				$ThisWhere = "WHERE files.type ='".$_GET['startvalue']."' AND files.approved ='no' AND members.verify_status ='1'"; 
		...
		//	if(isset($_GET['startvalue']) && $_GET['startvalue'] !="" && $_GET['startvalue'] !="0"){ $ThisWhere = "WHERE members.active='".$_GET['startvalue']."'"; }else{ $ThisWhere=""; }
		if($_GET['startvalue']=="0" ){ 
			$ThisWhere = "WHERE (members.active='active' OR members.active='suspended') AND members.verify_status !='0'";
			
			$GLOBALS['SEARCH_DATA'] = array( //Full Texts  	id 	caption 	forder 	private
			"tb_system" => "members",  
			"tb_captions" => array("ID","Main Photo","Username","Popular","Age","Name","Phone","Approved","activate_code","Last Login","Account Created","Gender","Email","Visible?","Membership","Country","Location","IP","Moderator"),
			"tb_data" =>  array("members.id","files.bigimage AS Photo","members.username","members.popular", "members_data.age", "members.firstname","members.phone","members.active", "members.activate_code","members.lastlogin", "members.created","members_data.gender", "members.email", "members.visible", "package.name AS Membership", "members_data.country","members_data.location", "members.ip","members.moderator"),
			//"tb_data" =>  array("members.id","files.bigimage AS Photo","(select files.bigimage2 from files where files.uid = members.id and files.bigimage2 != '' limit 1) AS Verify","members.username", "members_data.age", "members_data.firstname","members_data.phone","members.active", "members.activate_code","members.lastlogin", "members.created","members_data.gender", "members.email", "members.visible", "package.name AS Membership", "members_data.country","members_data.location", "members.ip","members.moderator"),
			"tb_data_name" => array("members.","members_data.","members_privacy."), // used to display the table data			 
			"tb_tables" => "members INNER JOIN members_data ON ( members.id = members_data.uid AND members_data.uid != 0 ) LEFT JOIN package ON (members.packageid = package.pid) LEFT JOIN files ON ( members.id = files.uid AND files.type='photo' AND files.default=1 ) ",
			"tb_where" => $ThisWhere, //WHERE files.album_id = albums.id
		...
		elseif($_GET['startvalue']=="unapproved" ) {
		$ThisWhere = "WHERE members.active='unapproved' AND members.verify_status !='0'"; 
		$GLOBALS['SEARCH_DATA'] = array( //Full Texts  	id 	caption 	forder 	private
			"tb_system" => "members",  
			"tb_captions" => array("ID","Main Photo","Verify Photo","Username","Age","Approval","Name","Phone","activate_code","Last Login","Account Created","Gender","Email","Visible?","Membership","Country","Location","IP","Moderator"),
			"tb_data" =>  array("members.id","files.bigimage AS Photo","files.bigimage2 AS Verify","members.username", "members_data.age","members.active AS NewActive", "members.firstname","members.phone", "members.activate_code","members.lastlogin", "members.created","members_data.gender", "members.email", "members.visible", "package.name AS Membership", "members_data.country","members_data.location", "members.ip","members.moderator"),
			//"tb_data" =>  array("members.id","files.bigimage AS Photo","(select files.bigimage2 from files where files.uid = members.id and files.bigimage2 != '' limit 1) AS Verify","members.username", "members_data.age", "members_data.firstname","members_data.phone","members.active", "members.activate_code","members.lastlogin", "members.created","members_data.gender", "members.email", "members.visible", "package.name AS Membership", "members_data.country","members_data.location", "members.ip","members.moderator"),
			"tb_data_name" => array("members.","members_data.","members_privacy."), // used to display the table data			 
			"tb_tables" => "members INNER JOIN members_data ON ( members.id = members_data.uid AND members_data.uid != 0 ) LEFT JOIN package ON (members.packageid = package.pid) LEFT JOIN files ON ( members.id = files.uid AND files.type='photo' AND files.default=1 ) ",
			"tb_where" => $ThisWhere, //WHERE files.album_id = albums.id
			...
			$GLOBALS['SEARCH_DATA'] = array( //Full Texts  	id 	caption 	forder 	private
			"tb_system" => "members",  
			"tb_captions" => array("ID","Main Photo","Username","Age","Name","Phone","Approved","activate_code","Last Login","Account Created","Gender","Email","Visible?","Membership","Country","Location","IP","Moderator"),
			"tb_data" =>  array("members.id","files.bigimage AS Photo","members.username", "members_data.age", "members.firstname","members.phone","members.active", "members.activate_code","members.lastlogin", "members.created","members_data.gender", "members.email", "members.visible", "package.name AS Membership", "members_data.country","members_data.location", "members.ip","members.moderator"),
			//"tb_data" =>  array("members.id","files.bigimage AS Photo","(select files.bigimage2 from files where files.uid = members.id and files.bigimage2 != '' limit 1) AS Verify","members.username", "members_data.age", "members_data.firstname","members_data.phone","members.active", "members.activate_code","members.lastlogin", "members.created","members_data.gender", "members.email", "members.visible", "package.name AS Membership", "members_data.country","members_data.location", "members.ip","members.moderator"),
			"tb_data_name" => array("members.","members_data.","members_privacy."), // used to display the table data			 
			"tb_tables" => "members INNER JOIN members_data ON ( members.id = members_data.uid AND members_data.uid != 0 ) LEFT JOIN package ON (members.packageid = package.pid) LEFT JOIN files ON ( members.id = files.uid AND files.type='photo' AND files.default=1 ) ",
			"tb_where" => $ThisWhere, //WHERE files.album_id = albums.id
        ...
        case "coupon":
            $GLOBALS['SEARCH_DATA'] = array( //Full Texts  	id 	caption 	forder 	private
                "tb_system" =>"coupon",
                "tb_captions" => array("ID","Redemption Code","Percentage Off","Active"),
                "tb_data" => array("coupon.id","coupon.code","coupon.per","coupon.enable"),
                "tb_data_name" => array("coupon."), // used to display the table data
                "tb_tables" => "coupon",
                "tb_where" => "", //WHERE files.album_id = albums.id
                "tb_OrderBy" => "coupon.id",
                "tb_OrderWay" =>"DESC",
                "tb_defaultField" => "coupon.code", // default search field
                "tb_deletevalue" => "coupon.id", // default search field
                "tb_edit" => false, // default search field
                "tb_edit_path" => "",
                "tb_rowsPerPage" => "20",
                "tb_hideID" => true,
