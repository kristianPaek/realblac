...
function GetProfileData($id, $selectedGroup="", $ShowTextGroupsOnly=0){
	...
	while( $field = $DB->NextRow($result1) ){
		...
		if($DataArray[$field['fName']] !="" && $DataArray[$field['fName']] !="0" && $field['fType'] != 5){
		$HasFieldConter++;
		}
...
$order   = array('\r\n', '\n', '\r', 'rn');
$replace = '';
$DataArray[$field['fName']] = $DataArray[$field['fName']];
							...
							$value .="<ul style='margin:0; padding:0; line-height: 1em; line-height:20px'>";
...
if ($mycount == 0) {
	...
	if($ShowTextGroupsOnly ==2){
							...
							// DISPLAY THE OUTPUT
							if($DataArray[$field['fName']] !=""){  
								$ReturnString .= '<div class="ClearAll"></div></div><div class="profile_box_body">';
								$ReturnString .= nl2br($DataArray[$field['fName']]); 
								$ReturnString .= "</div>"; 
							}
							...
							if($DataArray[$field['fName']] !="" && $DataArray[$field['fName']] !="0"){  
								$ReturnString .= '<b><div style="width:185px; float:left; min-height:8px;">'.$Caption['caption'].'</b>:&nbsp;<span>';
								$ReturnString .= $DataArray[$field['fName']]."</span></div>"; 
							}
	}
