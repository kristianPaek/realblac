...
function HSVupdate(v) { v=color.HSV_HEX(HSV=v?v:slideHSV);
	...
	var ThisBox = document.getElementById('ColourPickIder').value;
	
 
	$('plugHEX').innerHTML=v;
	$S('plugCUR').background='#'+v;
	//if(ThisBox !="ColourPickIder"){ UpdateFrame(ThisBox, v); }

	var ThisBox2 = document.getElementById(ThisBox);

	document.getElementById(ThisBox).value = '#'+v;
	document.getElementById(ThisBox).style.background = '#'+v; 
...
function getColor(coloridbox) {
	
	document.getElementById('ColourPickIder').value = coloridbox;

	var ThisBox = document.getElementById('ColourPickIder').value;
	var ThisBox2 = document.getElementById(ThisBox);
 	

	$S('plugin').left=(ThisBox2.offsetLeft+400)+'px';
	$S('plugin').top=(ThisBox2.offsetTop+770)+'px';	
	toggle('plugin');
