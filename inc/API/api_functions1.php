...
function MyLastVisitedProfile($id, $limit=1){
		...
		$result = $DB->Query("SELECT max(visited.autoid), members.id, members.username, members_data.age, members_data.gender,  members_data.country, album.cat, files.type, files.adult_content, files.approved, files.bigimage 
		FROM visited
