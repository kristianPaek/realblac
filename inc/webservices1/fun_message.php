        ...
        for($i=0;$i<14;$i++) {
            ...
            if(mysql_num_rows($Data)>0){
                ...
                while($row=mysql_fetch_array($Data)){
					   $checkBlock = "SELECT * FROM `members_network` WHERE ((uid='".$row['id']."' AND to_uid ='".$id."') OR (uid ='".$id."' AND to_uid ='".$row['id']."')) AND approved='yes'";
                    ...
                    $messages[$date][] = array("date" => $date,"wink_id"=>$row['wsid'], 'id'=>$row['id'],'username'=>$row['username'],'active'=>$row['active'],'view_status'=>$row['read_status'],'block_status'=>$block_status);
...
function GetMessageDate($id,$box=1, $orderBy="maildate", $Start="0", $Stop="10"){
...
function SendWinkMessageNew($sendfrom,$sendto){
    ...
    if($Total['countwink']==0){
     ...
     $CheckWink = $DB->Row("SELECT * FROM members_privacy WHERE `uid` = '".$CurrentRel['id']."' LIMIT 1");
		
		if(($CheckWink['email_winks'])=="yes")
		{
