...
function GetMessage($id,$box=1, $orderBy="maildate", $start="0", $stop="10"){
    ...
    while ($mail_room = $DB->NextRow($mail_rooms)) {
    	...
    	if ($box == "sent") {
    		...
    		$SQL = "SELECT members.username,members.active, messages.mailnum, messages.mailtime, messages.mail_message, messages.mail2id, messages.uid, messages.mailstatus, messages.type, messages.maildate, messages.mail_subject, files.bigimage, files.type, album.cat, album.allow_a,	album.allow_n,	album.allow_h,	album.allow_f
                FROM messages
                LEFT JOIN files ON ( files.uid = messages.mail2id AND files.approved = 'yes' AND files.`featured` ='yes' AND files.title != 'Message Photo' AND files.default=1)
    		...
    		$SQL = "SELECT members.username,members.active, messages.mail_message, messages.mailtime, messages.mailnum, messages.mail2id, messages.uid, messages.mailstatus, messages.type, messages.maildate, messages.mail_subject, files.bigimage, files.type, album.cat, album.allow_a,	album.allow_n,	album.allow_h,	album.allow_f
                FROM messages
                LEFT JOIN files ON ( files.uid = messages.mail2id AND files.approved = 'yes' AND files.title != 'Message Photo' AND files.`featured` ='yes' )
                LEFT JOIN album ON ( album.aid = files.aid )
                LEFT JOIN members ON ( members.id = messages.mail2id )
                WHERE messages.uid = " . $id . " AND messages.mail_room_id = ". $mail_room['id'] . " ORDER BY mailnum DESC LIMIT 1";
    	}
            ...
            if ($mail_room['room_my_status'] == 'close' || $mail_room['room_partner_status'] == 'close')
    		 ...
    		 $SQL = "SELECT members.username,members.active, messages.mailnum, messages.mailtime, messages.mail_message, messages.mail2id, messages.uid, messages.mailstatus, messages.type, messages.maildate, messages.mail_subject, files.bigimage, files.type, album.cat, album.allow_a, album.allow_n,	album.allow_h,	album.allow_f
                FROM messages
                LEFT JOIN files ON ( files.uid = messages.uid AND files.approved = 'yes' AND files.title != 'Message Photo' AND files.`featured` ='yes' )
                LEFT JOIN album ON ( album.aid = files.aid )
...
$checkBlock = "SELECT * FROM `members_network` WHERE ((uid='".$senderid."' AND to_uid ='".$id."') OR (uid ='".$id."' AND to_uid ='".$senderid."')) AND approved='yes'";
...
&nbsp;<p>Good luck and happy dating!</p><p>RBL Team&nbsp;</p><div align="left"><p style="margin:0px!important;color:#999999;font-size:13px"><font size="3">&nbsp;</font></p></div></div></td></tr></tbody></table></td><td class="m_-171776263396577492msib-right-img m_-171776263396577492rnb-force-col" width="178" valign="top" style="padding-left:20px"><br></td></tr></tbody></table></td></tr><tr><td height="30" style="font-size:1px;line-height:1px">&nbsp;</td></tr></tbody></table></td></tr></tbody></table></td></tr><tr><td align="center" valign="top" bgcolor="#ffffff" style="background-color:#ffffff"><table class="m_-171776263396577492rnb-del-min-width" width="100%" cellpadding="0" border="0" cellspacing="0" bgcolor="#ffffff" name="Layout_12" id="m_-171776263396577492Layout_12" style="border-spacing:0px;min-width:0px!important"><tbody><tr><td class="m_-171776263396577492rnb-del-min-width" align="center" valign="top" bgcolor="#ffffff" style="min-width:0px!important"></td></tr></tbody></table></td></tr><tr><td align="center" valign="top" bgcolor="#ffffff" style="background-color:#ffffff"></td></tr><tr><td align="center" valign="top" bgcolor="#ffffff" style="background-color:#ffffff"><br></td></tr></tbody></table></body></html>';  
    ...
    foreach($ToArray as $receiver_name){
        ...
        //$UserName = $DB->Row("SELECT id FROM members WHERE id = '" . eMeetingInput($receiver_name) . "' LIMIT 1");
...
/*	if( $cDate < $NewDate &&  D_FREE =="no" && $result['packageid'] == 3 )
	{
            ...
            if($blocked['total'] ==0){
                ...
                //if($blocked['total'] ==0){
                    ...
                    DoEmailSMS($receiver_id, 5, 'email_msg', substr($values['message1'], 0, 9000));
