...
$CheckUser = "SELECT * FROM `members` WHERE id='".$id."'";
$CheckData = mysql_fetch_assoc( mysql_query($CheckUser) );

		if(($CheckData['packageid'] == "68") || ($CheckData['packageid'] == "70"))
	{
		
        $result = $DB->Query("SELECT album.cat, files.aid, files.type, files.adult_content, files.approved, files.bigimage,members.active, members.username,members.id, visited.autoid, visited.uid, visited.date
			FROM
			members
			...
			WHERE visited.view_uid='".$id."' AND visited.date LIKE '%".$SearchDate."%' AND members.id NOT IN('".$UserDataM."') GROUP BY visited.uid ORDER BY visited.date DESC LIMIT 200");
	}
	else{
   ...
   $result = $DB->Query("SELECT album.cat, files.aid, files.type, files.adult_content, files.approved, files.bigimage,members.active, members.username,members.id, visited.autoid, visited.uid, visited.date
			FROM
			members
			INNER JOIN visited ON (visited.uid = members.id  AND members.packageid !='70' AND members.packageid !='68' AND visited.uid != ".$id.")
			LEFT JOIN files ON (files.uid = members.id AND files.default=1)
			LEFT JOIN album ON (album.aid = files.aid)
			WHERE visited.view_uid='".$id."' AND visited.date LIKE '%".$SearchDate."%' AND members.id NOT IN('".$UserDataM."') GROUP BY visited.uid ORDER BY visited.date DESC LIMIT 200");
        ...
        while( $history = $DB->NextRow($result) ) {
			...
			$checkBlock = "SELECT * FROM `members_network` WHERE ((uid='".$history['id']."' AND to_uid ='".$id."') OR (uid ='".$id."' AND to_uid ='".$history['id']."')) AND approved='yes'";
					
					$queryyy = mysql_query($checkBlock);
				 	$row2 = mysql_num_rows($queryyy);
     ...
     /*       if($history['cat'] != "public" && $history['id'] != $id) {
			 ...
			 $result5 = $DB->Row("SELECT type, adult_content, bigimage, files.approved FROM files WHERE uid='".$history['id']."' AND type='photo' and adult_content !='yes' and approved ='yes' and featured ='yes' ORDER BY `default` DESC LIMIT 1");
			
			 if($result5['bigimage']) {
                $pimage		= 	WEB_PATH_IMAGE.$result5['bigimage'];
              }
			  else{
				 $pimage		= 	WEB_PATH_IMAGE."waiting.jpg";
			  }
...
function putMayEditProfileData($id,$values) {
    ...
    //return $_SESSION['uid'];
    //echo "SELECT id FROM members_edit_temp WHERE `id` = ('".$_SESSION['uid']."') LIMIT 1";
...
function getMayProfileData($id) {
    ...
    $sql = "SELECT members.activate_code,members.username,members.firstname,members.phone, members_template.header_background AS background, members_template.header_text AS color_text, members_data.location AS locationVal, members_data.gender AS genderD, package.view_adult, package.name, package.wink, package.Highlighted, package.Featured, package.maxMessage, members.moderator, package.maxFiles, members.active, members.id, members.activate_code, members.username, members.packageid,members.session,members.email , members.lastlogin, members_privacy.Language FROM members
	...
	$result1 =("SELECT * FROM files WHERE uid='".$values['id']."' AND adult_content !='yes' AND type='photo' AND approved='yes' ORDER BY `default` DESC LIMIT 1");	
	$result12 = mysql_fetch_assoc( mysql_query($result1) );
	$approvedStatus = $result12['approved'];
	$result3 =("SELECT * FROM members_edit_temp WHERE id='".$values['id']."' ");	
	$result33 = mysql_fetch_assoc( mysql_query($result3) );
    $Description =("SELECT * FROM members_data WHERE uid='".$values['id']."' ");	
	$Description11 = mysql_fetch_assoc( mysql_query($Description) );
	if($Description11['em_7s920130723'] == "5772"){
		$kids= "None";
	}
	if($Description11['em_7s920130723'] == "5766"){
		$kids= "1 child";
	}
	if($Description11['em_7s920130723'] == "5767"){
		$kids= "2 children";
	}
	if($Description11['em_7s920130723'] == "5768"){
		$kids= "3 children";
	}
	if($Description11['em_7s920130723'] == "5769"){
		$kids= "4 children";
	}
	if($Description11['em_7s920130723'] == "5770"){
		$kids= "5 children";
	}
	if($Description11['em_7s920130723'] == "5771"){
		$kids= "6  children";
	}
	if($Description11['em_7s920130723'] == "5612"){
		$kids= "Yes / Full-time";
	}
	if($Description11['em_7s920130723'] == "5613"){
		$kids= "Yes / Part-time";
	}
	if($Description11['em_7s920130723'] == "5614"){
		$kids= "Yes / Grown";
	}
	if($Description11['em_7s920130723'] == "5715"){
		$kids= "None / Want";
	}
	if($Description11['em_7s920130723'] == "5716"){
		$kids= "None / Don't Want";
	}
   ...
   /* $album = $DB->Query("select * from album where uid = ".$profileId);
    ...
    while( $Data = $DB->NextRow($album) ) {
	   ...
	   $album = $DB->Query("select album.uid,album.title,album.aid,files.uid,COUNT(files.approved) AS total from album LEFT JOIN files ON album.uid=files.uid where files.approved ='yes' AND album.uid = ".$profileId);
    $albums = array();
    while( $Data = $DB->NextRow($album) ) {
        $albums[] = array(
                "album_id"=>$Data['aid'],
                "photo_count"=>$Data['total'],
                "title"      => $Data['title']
        );
    }
...
if(!empty($result12['bigimage']))
{
 ...
 if($result33['moderator']=="no") {
         $AboutMe = "";
	 //$AboutMe = $Description11['description'];
   }
	else{
		 $AboutMe = $Description11['description'];
		 //$AboutMe = "";
...
$finalProfile=array("profileImage"=> $ProfilePic,"approved"=> $approvedStatus,"About me main"=> $AboutMe,"kids"=>$kids, "albums"=>$albums,"LocationName"=>$locationName,"active_status"=>$active_status);
        ...
        while( $field = $DB->NextRow($result1) ) {
            ...
            if($field['fType'] == 2) {
                ...
                if(isset($DataArray[$field['fName']]) && $DataArray[$field['fName']]!="") {
                    $dataArray['username']['username']=$values['username'];
