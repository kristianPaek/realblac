 ...
 */
function UploadFileNew($FILE, $logid, $title, $desc, $default_image, $type, $aid, $adult_file){
    ...
    $username_result = $DB->Row("SELECT username,email,id,packageid FROM members WHERE id='".$logid."' LIMIT 1");

    if($logid != 0 && D_FREE =="no" && !isset($_SESSION['admin_access_level']) ){

        $sp = $DB->Row("SELECT sum(maxFiles) AS space FROM package WHERE pid=( '".$username_result['packageid']."' )");
        ...
        if( ($usedimagespace['space'] > $sp['space']) && !isset($_SESSION['admin_access_level']) ){
            if($username_result['packageid']==3){
                return "Limited members can only uploads five photos. If you want to upload more photos please upgrade to \"VIP Member\"";
            }
            elseif($username_result['packageid'] == 19){
                return "You can not upload any more photos";
            }
            else {
                return "You can upload only 25 photos";
            }
        }
    }
...
//        $eType="This image has bad format.";
//    }
//
//    if($CanContinue !='1'){
...
//    }
    ...
    $rn = rand(10000,99999);
    $randomImageFileName = $rn.substr($FILE['name'], -5);

    $copy = copy($FILE['tmp_name'], $UploadPath.$randomImageFileName);

    //$copy = copy($FILE['tmp_name'], $UploadPath.$FILE['name']);


    if($copy){
        //print_r($UploadPath.$FILE['name']);
        /**
        ...
        if($sizes[0] > 350 && $type=="photo"){
            ...
            $new_height = $sizes[1];
            $CanContinue = PhotoThumbNail($randomImageFileName,$FILE,0, 1, $new_height);
        }
        ...
        $img_array= explode('.', $randomImageFileName);

        if(rename_win($UploadPath.$randomImageFileName,$UploadPath."v9_".$newname.".".strtolower($img_array[1])) == FALSE) {

            $image_name = $randomImageFileName;
        ...
        if($aid =="new"){
            $DB->Insert("INSERT INTO `album` ( `aid` , `uid` , `title` , `comment` , `filecount` , `cat` , `allow_f` , `allow_h` , `allow_n` , `allow_a`,password )
											VALUES (NULL , '".$logid."', '".$username_result['username']." Photo Album', '', '0', 'public', '1', '1', '1', '1','')");
            $aid = $DB->InsertID();
        }
...
//AddEventSystemLog($_SESSION['username'],"comment_".$page, $page, $sub, $id1, $id2,$id3);
            AddEventSystemLog(eMeetingInput($username_result['username']),"file_add", "", "", $logid, $photo_id,$image_name, 0);
        ...
        if(APPROVE_FILES == "yes"){
            ...
            $Data['custom'] = $username_result['username'];
            $Data['username'] = $username_result['username'];
        ...
        return "Image can not upload at this time. Please try again."; // image didnt upload
    }
...
function PhotoThumbNail($image_name, $photo_upload,$photo_id, $ResizeLarge, $LH=''){
    ...
    global $DB;
    include_once("../classes/class_thumbnail.php");
