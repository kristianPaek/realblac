...
function showAlbumPhotos($id,$albumOwnerID, $aid, $pass, $profile=false){
    ...
    $SQL = "SELECT album.cat, album.allow_f, album.allow_h, files.default, files.description, album.time, album.date, album.title AS atitle, files.adult_content,  files.approved,   files.bigimage,files.featured,  files.id,  files.uid, files.aid,  files.type,  files.title,  files.views , files.rating,  files.rating_votes
	FROM files
