...
elseif($action == "GET_MAIL"){
    ...
    $Date = GetMessageDate($user_id, $box, $sort_by, $start, $end);
    echo json_encode(array("message" => "SUCCESS", "method" => "GET_MAIL", "mail_records"=>$messages, "dates"=>$Date));
}
...
elseif($action == "MY_PROFILE"){
	...
	$CheckPlan = $DB->Row("SELECT * FROM members WHERE id='".$user_id."' LIMIT 1");
	$CheckPlanVisitor = $DB->Row("SELECT * FROM members WHERE id='".$visitor_uid."' LIMIT 1");
	
	$MatchPlan =  $CheckPlan['packageid'];
	
	
    if((!empty($visitor_uid) && $user_id !=$visitor_uid) )
    {
	    if((($CheckPlan['packageid'] =="3") || ($CheckPlan['packageid'] =="19") || ($CheckPlan['packageid'] =="54")|| ($CheckPlan['packageid'] =="18")) && (($CheckPlanVisitor['packageid'] =="68") || ($CheckPlanVisitor['packageid'] =="70")))
		{
		   
		}
		elseif((($CheckPlan['packageid'] =="3") || ($CheckPlan['packageid'] =="19") || ($CheckPlan['packageid'] =="54")|| ($CheckPlan['packageid'] =="18")) && (($CheckPlanVisitor['packageid'] !=="68") || ($CheckPlanVisitor['packageid'] !=="70")))
		{
			
			 $DB->Insert("INSERT INTO `visited` (uid ,view_uid ,date)VALUES ('".$visitor_uid."', '".$user_id ."', NOW())");
		}
		elseif(($CheckPlanVisitor['packageid'] =="68") || ($CheckPlanVisitor['packageid'] =="70") && ($CheckPlan['packageid'] =="68") || ($CheckPlan['packageid'] =="70")){
...
elseif($action == "GET_EDIT_PROFILE"){
    ...
    $Getdetail =("SELECT * FROM `members` WHERE id='".$user_id."'");	
	$GetdetailOne = mysql_fetch_assoc( mysql_query($Getdetail) );
	
    include_once("fun_profile.php");
    $profileData = getMayEditProfileData($user_id);
    echo json_encode(array("message" => "SUCCESS", "method" => "GET_EDIT_PROFILE","firstname"=>$GetdetailOne['firstname'],"phone"=>$GetdetailOne['phone'], "profile"=>$profileData));
}
...
elseif($action == "EDIT_MY_PROFILE"){
	//MEMBERS table use only
    $user_id = trim(strip_tags($_POST['user_id']));       //User Id
    //$username = trim(strip_tags($_POST['username']));     //User Name
    //$firstname = trim(strip_tags($_POST['firstname']));   //First Name
   // $phone = trim(strip_tags($_POST['phone']));           //Phone Number
	
	//MEMBERS_DATA table use only
	$State = trim(strip_tags($_POST['FieldValue54']));              //State                  em_85820081128 
	$Prefer = trim(strip_tags($_POST['FieldValue92']));             //Prefer                 em_5jb20131011  
    ...
    $age = $year."-".$month."-".$date;                              //Full Date
	$Race = trim(strip_tags($_POST['race']));                       // Race                  em_yh020080113 
	$Height = trim(strip_tags($_POST['FieldValue31']));             // Height                em_1k820080113  
	$BodyType = trim(strip_tags($_POST['FieldValue32']));           // BodyType              em_heh20080113  
	$Status = trim(strip_tags($_POST['MaritalStatus']));            // Status                em_hrh20080113  
	$Religion = trim(strip_tags($_POST['Religion']));               // Religion              em_txg20080113  
	$Education = trim(strip_tags($_POST['FieldValue48']));          // Education             em_s1620080113
	$Work = trim(strip_tags($_POST['Work']));                       // Work                  em_72220080113
	$Income = trim(strip_tags($_POST['FieldValue49']));             // Income                em_kjc20080113
	$Kids2 = trim(strip_tags($_POST['kids2']));                     // Kid(s)                em_7s920130723
	$Marriage = trim(strip_tags($_POST['Marriage']));               // Marriage              em_s5j20080113
	$Description = trim(strip_tags($_POST['description']));         // Description           description
	$Smoking = trim(strip_tags($_POST['Smoking']));                 // Smoking               em_1cz20131006
	$Drinking = trim(strip_tags($_POST['Drinking']));               // Drinking              em_7gb20131006
	$Personality = trim(strip_tags($_POST['Personality']));         // Personality           em_wvh20080113
	$Politics = trim(strip_tags($_POST['Politics']));               // Politics              em_hes20130723
	$Romantic = trim(strip_tags($_POST['Romantic']));               // Romantic              em_r9720080113
	$Cooking = trim(strip_tags($_POST['Cooking']));                 // Cooking               em_ja520130723
	$Musical = trim(strip_tags($_POST['Musical']));                 // Musical Taste         em_y8520080116
	$Movies = trim(strip_tags($_POST['Movies']));                   // Movies                em_grm20080116
	$Hobbies = trim(strip_tags($_POST['Hobbies']));                 // Hobbies\/Pastimes     em_8hs20130723
	$Hair_Color = trim(strip_tags($_POST['Hair_Color']));           // Hair Color            em_93n20080113
	$Hair = trim(strip_tags($_POST['Hair']));                       // Hair                  em_jhb20080113
	$Eye_Color = trim(strip_tags($_POST['Eye_Color']));             // Eye Color             em_jsh20080113
	$Best_Feature = trim(strip_tags($_POST['Best_Feature']));       // Best Feature          em_vqf20080113
	$Car_Owner = trim(strip_tags($_POST['Car_Owner']));             // Car Owner             em_q1420130723
	
	if(!empty($user_id))
	{
		
		//$DB->Insert("UPDATE members SET username ='".$username."' WHERE id= ( '".$user_id."' ) LIMIT 1");
		
		$DB->Insert("UPDATE members_data SET em_85820081128 ='".$State."',em_5jb20131011 ='".$Prefer."',age ='".$age."',em_yh020080113 ='".$Race."',em_1k820080113 ='".$Height."',em_heh20080113 ='".$BodyType."',em_hrh20080113 ='".$Status."',em_txg20080113 ='".$Religion."',em_s1620080113 ='".$Education."',em_72220080113 ='".$Work."',em_kjc20080113 ='".$Income."',em_7s920130723 ='".$Kids2."',em_s5j20080113 ='".$Marriage."',description ='".$Description."',em_1cz20131006 ='".$Smoking."',em_7gb20131006 ='".$Drinking."',em_wvh20080113 ='".$Personality."',em_hes20130723 ='".$Politics."',em_r9720080113 ='".$Romantic."',em_ja520130723 ='".$Cooking."',em_y8520080116 ='".$Musical."',em_grm20080116 ='".$Movies."',em_8hs20130723 ='".$Hobbies."',em_93n20080113 ='".$Hair_Color."',em_jhb20080113 ='".$Hair."',em_jsh20080113 ='".$Eye_Color."',em_vqf20080113 ='".$Best_Feature."',em_q1420130723 ='".$Car_Owner."' WHERE uid= ( '".$user_id."' ) LIMIT 1");
		
		
		
		$FoundEditTemp = $DB->Row("SELECT id FROM members_edit_temp WHERE `id` = ('".$user_id."') LIMIT 1");
		 $FoundEditTemp2 = $DB->Row("SELECT uid FROM members_data_edit_temp WHERE `uid` = ('".$user_id."') LIMIT 1");
	
		if(!isset($FoundEditTemp2['uid'])){
			$DB->Insert("INSERT INTO members_data_edit_temp SELECT * FROM members_data WHERE uid = ( '".$user_id."' ) LIMIT 1");
		}
		
		if(!isset($FoundEditTemp['id'])) {
			$DB->Insert("INSERT INTO members_edit_temp SELECT * FROM members WHERE id= ( '".$user_id."' ) LIMIT 1");
         }
         $DB->Update("UPDATE members_edit_temp SET updated='".DATE_TIME."',active='unapproved' WHERE id= ( '".$user_id."' ) LIMIT 1");
			$DB->Update("UPDATE members_data_edit_temp SET uid= ( '".$user_id."' )  WHERE uid = ( '".$user_id."' ) LIMIT 1");
		

		echo json_encode(array("message" => "Profile Updated Successfully","status" => "true", "method" => "EDIT_MY_PROFILE"));
	}
	else{
		echo json_encode(array("message" => "Input Error","status" => "false", "method" => "EDIT_MY_PROFILE"));
	}
...
elseif($action == "SEND_WINK"){
    ...
    $CurrentRel = $DB->Row("SELECT id,email,username FROM members WHERE username='".$sendto."' LIMIT 1");
    ...
    $message=$sendfrom.' winked you';
        ...
        $registatoin_ids = array($device['udid']);
			$message =array("body" =>array(
			"alert" => $message,
			"user_id" => $userID['id'], 
			"sender_user_name" => $sendfrom,
			"type" => 'Wink'
		));
        //$message = array("alert" => $message,"user_id" => $userID['id'],"sender_user_name" => $sendfrom,"type" => 'Wink');
...
elseif($action == "UPLOAD_PHOTO_ON_SERVER"){
    ...
    $default    = 0;
    $upAdult    = "no";
$D1 = $DB->Row("SELECT * FROM files WHERE bigimage2 !='' AND uid='".$userID."' LIMIT 1");
$DB->Update("UPDATE files SET `bigimage2`= '".$D1['bigimage2']."' WHERE uid= ( '".$userid."' ) ");
...
elseif($action == "SUBSCRIPTION_DETAILS"){
	...
	//$userID_result = $DB->Row("SELECT * FROM members_billing WHERE uid='".$userID."' LIMIT 1");
	$userID_result = $DB->Row("SELECT * FROM members_billing WHERE uid='".$userID."' ORDER BY id DESC LIMIT 1");
    $packageid = $userID_result['packageid'];
	
	
	$AdminAuthorize = $DB->Row("SELECT * FROM authorise WHERE authorise_id='1' LIMIT 1");
    $AdminName = base64_decode($AdminAuthorize['username']);
    $AdminPassword = base64_decode($AdminAuthorize['password']);
	
    $date_expire = $userID_result['date_expire'];
	## DISPLAY UPGRADE Date
	$DateTime  = $userID_result['date_upgrade'];
	$Date = date('Y:m:d', strtotime($DateTime));
	$date_expire1 = date('Y:m:d', strtotime($date_expire));
    ...
    $PageDesc['upgraded_date'] = !empty($DateTime) ? $Date : "";
    $PageDesc['packageid'] = !empty($packageid) ? $packageid : "";
    $PageDesc['date_expire'] = !empty($date_expire) ? $date_expire1 : "";
    $PageDesc['authusername'] = !empty($AdminName) ? $AdminName : "";
    $PageDesc['authpassword'] = !empty($AdminPassword) ? $AdminPassword : "";
...
elseif($action == "SUBSCRIPTION_DATA_SAVED_ON_SERVER"){
	...
	$DB->Update("UPDATE members SET subscription_id='".$subscriptionID."' WHERE id='".$userID."'");
