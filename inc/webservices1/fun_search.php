...
$UserDataM	= implode("','", $UserData1);
   ...
   // Changed by salentro
    if($site_moderator_approve=="yes" ){
        ...
        if(($privacy['packageid'] == "68") || ($privacy['packageid'] == "70"))
		{
    ...
    if(isset($pageGET['Extra']['period']) && is_numeric($pageGET['Extra']['period']) && $pageGET['Extra']['period'] > 1){
        $RunExtra1 .= " TO_DAYS( NOW(  'y-m-d'  )  )  - TO_DAYS( members.created )  < ".$pageGET['Extra']['period']." AND ";
    }
    ...
    if(isset($pageGET['Extra']['newtoday']) && $pageGET['Extra']['newtoday']==1){
        $RunExtra1 .= " TO_DAYS( NOW(  'y-m-d'  )  )  - TO_DAYS( members.created )  < 2 AND ";
    }
    ...
    if(isset($pageGET['Extra']['period']) && is_numeric($pageGET['Extra']['period']) && $pageGET['Extra']['period'] > 1){
        ...
        $QueryTotal .= " AND TO_DAYS( NOW(  'y-m-d'  )  )  - TO_DAYS( members.created )  < ".$pageGET['Extra']['period']."  ";
    ...
    if(isset($pageGET['Extra']['newtoday']) && $pageGET['Extra']['newtoday']==1){
        ...
        $QueryTotal .= " AND TO_DAYS( NOW(  'y-m-d'  )  )  - TO_DAYS( members.created )  < 2 ";
...
//print $QueryTotal;
    ...
    $QQ .="WHERE $RunExtra1 members.email !='' AND members.visible = 'yes' AND members.id NOT IN('".$UserDataM."') ";
...
$newDataArray = array();
 ...
 $result5 = $DB->Row("SELECT type, adult_content, bigimage, files.approved FROM files WHERE uid='".$Data['id']."' AND type='photo' and adult_content !='yes' and approved ='yes' and featured ='yes' ORDER BY `default` DESC LIMIT 1");
        ...
        if($Data['id'] != $id){
            $newDataArray[] = array(
                ...
                "onlinenow"         => $OnlineM,
                //  "image"             => $image_url,
                "image"             => WEB_PATH_IMAGE.$result5['bigimage'],
                "video"             => $video,
                "icon"              => $icon_url,
