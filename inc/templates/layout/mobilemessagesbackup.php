...
<li>Email subject (Don't change original subject title)<input name="subject" id="SentSubject" type="text" size="35" class="input" style="width:280px" value="<? if (isset($msg_subject)) { print $msg_subject; }  if (isset($_GET['msg_subject'])) { print eMeetingInput($_GET['msg_subject']); } if (isset($_POST['msg_subject'])) { print eMeetingInput($_POST['msg_subject']); } ?>"></li>
...
<textarea name='message1' id="formMessage1" rows=5 class="input" readonly style="width:290px"><? if (isset($msg_content)) { print eMeetingInput($msg_content); } if(isset($_GET['msgid'])) {  echo GetMsgFromId($_GET['msgid']); }  ?></textarea>
               <br/ ><br /> <li>Type message to be sent in box below <textarea name='message' id="formMessage" rows=4 class="input" style="width:290px"></textarea>
...
<div id="Display_Message">
	...
	<div id="messages_read22" class="small">
		...
		<span>
			...
			<p style="line-height:20px; text-align:justify;"><?=$msgdata[1]['message']; ?></p>

			<p><small>Original message sent on <?=$GLOBALS['_LANG']['_date'] ?> <?=$msgdata[1]['date']; ?></small></p>			
