...
<a href="<?=DB_DOMAIN ?>index.php?dll=search&sub=advanced" style="text-decoration:none;"><img src="/searchmembers.png"></a> &nbsp; <a <? if($_SESSION['auth'] =="yes"){ ?> onclick="javascript:SavePage();" href="#" <? }else{ ?> href="<?=DB_DOMAIN ?>index.php?dll=login" <? } ?> style="text-decoration:none;"> <img src="/savesearch.png"> </a>
</div>
...
?>
	...
	<div class="Acc_ListBox_left <? if($Member['featured']=="yes"){ ?>highlighted3<? } ?>" style="width:120px;height:150px;"><div class="pic75">
		...
		<br><b><?=$Member['username'] ?></b> <? if(D_FRIENDS==1222){ ?><br><a href="<?=DB_DOMAIN ?>index.php?dll=search&friendid=<?=$Member['id'] ?>"><?=$GLOBALS['_LANG']['_friendsList'] ?></a> <? } ?></div>
	</div></div>
   ...
   <? if(D_STARSIGN ==1){ ?>(<?=$Member['starsign'] ?>)<? } ?> / <? if($Member['genderID'] == 2710){?> <img src="<?=DB_DOMAIN ?>images/DEFAULT/_icons/couple.gif" align="absmiddle"><? } ?><?=$Member['gender'] ?> <br> <?=$Member['location'] ?>  <? if($Member['mystate']){ ?> <?=$Member['mystate'] ?> <? } ?> 
...
<li class="Stop"> <input type="checkbox" name="Extra[pics]" value="1">  <strong><?=$GLOBALS['_LANG']['_withPics'] ?></strong> </li>
<li class="Stop"> <input type="checkbox" name="Extra[online]" value="1"><strong><?=$GLOBALS['_LANG']['_online'] ?> <?=$GLOBALS['_LANG']['_now'] ?></strong></li>
<li class="Stop"> <input type="checkbox" name="Extra[newtoday]" value="1"><strong><?=$GLOBALS['_LANG']['_joinToday'] ?></strong> </li>   
<? if(FLASH_VIDEO =="yes"){ ?><li class="Stop"> <input type="checkbox" name="Extra[livevideo]" value="1"> <strong><?=$LANG_ACCOUNT_MENU['video'] ?></strong> </li>  <? } ?>
	...
	<li class="sub"><img src="<?=DB_DOMAIN ?>images/DEFAULT/_icons/new/user_green.png" width="16" height="16" align="absmiddle"> <a href="#" onClick="toggleLayer('SearchOp1I1'); return false;"><?=$GLOBALS['_LANG']['_menue6'] ?></a></li>
...
<li><select name="Extra[sort]" class="input" style="width:185px">		<option value="1">---> <?=$GLOBALS['_LANG']['_sort'] ?>  </option>	<option value="1"><?=$GLOBALS['_LANG']['_menue10'] ?></option>   <option value="2"><?=$GLOBALS['_LANG']['_photos'] ?></option> <option value="4"><?=$GLOBALS['_LANG']['_updated'] ?></option> <option value="5"><?=$GLOBALS['_LANG']['_username'] ?></option> <option value="6"><?=$GLOBALS['_LANG']['_gender'] ?></option>  <option value="7"><?=$GLOBALS['_LANG']['_age'] ?></option>  </select></li>
