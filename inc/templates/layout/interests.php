...
<div id="eMeeting" class="user">
  <div class="header account_tabs">
    <ul>
	 	<li><a href="<?=DB_DOMAIN ?>index.php?dll=interests&sub=manage"><span>Search Interests</span></a></li>

	 	<li <? if($show_page=="manage"){ ?>class="selected"<? } ?>><a href="<?=DB_DOMAIN ?>index.php?dll=interests&sub=manage"><span><?=$LANG_GROUPS_MENU['manage'] ?></span></a></li>
		<li <? if($show_page=="add"){ ?>class="selected"<? } ?>><a href="<?=DB_DOMAIN ?>index.php?dll=interests&sub=add"><span><?=$LANG_GROUPS_MENU['add'] ?></span></a></li>
    </ul>
...
.search_display_featured { background:#FFF8DD url('images/DEFAULT/featured.jpg') no-repeat bottom right; }
	...
	<div id="Title">
		<div class="AddIcon"><br><a href="<?=DB_DOMAIN ?>index.php?dll=interests&sub=add" class="MainBtn">  <img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/add.png" align="absmiddle"> <?=$GLOBALS['_LANG']['_createNew'] ?></a></div>
	...
	<form name="SearchResults" method="post" action="<?=DB_DOMAIN ?>index.php?dll=interests&sub=view&gid=<? if(isset($_GET['gid']) && is_numeric($_GET['gid'])){ print $_GET['gid']; } ?>">
	...
	<div style="padding:50px;line-height:30px;"><h1><a href="<?=DB_DOMAIN ?>index.php?dll=interests&sub=view"> <?=$GLOBALS['_LANG_ERROR']['_noResults'] ?></a></h1></div>
	...
	<img src="<?=DB_DOMAIN ?>images/DEFAULT/_icons/new/pencil.png" width="16" height="16" align="absmiddle"> <a href="<?=DB_DOMAIN ?>index.php?dll=interests&sub=add&eid=<?=$value['id'] ?>"> <?=$GLOBALS['_LANG']['_edit'] ?> </a> <br>
...
<? } ?>
	...
	<form method="post" name="MemberSearch" action="<?=DB_DOMAIN ?>index.php">                        
	<input name="do" type="hidden" value="add" class='hidden'>
	<input name="do_page" type="hidden" value="interests" class="hidden">
	...
	<ul class="form">   
		...
		<div class="CapBody">
			...
			<li><label><?=$GLOBALS['_LANG']['_category'] ?></label><select name="cat_id"  class="input"> <?=DisplayCatsList($data['cat_id']) ?>    </select>      </li>
...
<li><label>Region</label> <select class="input"><option></option></select> </li>
<li><label>Town</label> <select class="input"><option></option></select> </li>
...
<li><? if(!isset($data)){ $data['description']=""; } ?> 
            <textarea name="editor" id="editor" cols=32 rows=7 style="width:590px;height:200px;"><? if(isset($data)){ print $data['description']; } ?></textarea>
</li>
