...
<div id="eMeeting" class="user">
    ...
    <ul>
	 	<li <? if($show_page=="edit"){ ?>class="selected"<? } ?>><a href="<?=DB_DOMAIN ?>index.php?dll=account&sub=edit"><span><?=$SubSub_Lang['edit'] ?></span></a></li>
		<? if(D_DESIGNER ==1){ ?><li <? if($show_page=="design"){ ?>class="selected"<? } ?>><a href="<?=DB_DOMAIN ?>index.php?dll=account&sub=design"><span><?=$SubSub_Lang['design'] ?></span></a></li><? } ?>
		<? if(FLASH_VIDEO =="yes"){ ?><li <? if($show_page=="video"){ ?>class="selected"<? } ?>><a href="<?=DB_DOMAIN ?>index.php?dll=account&sub=video"><span><?=$SubSub_Lang['video'] ?></span></a></li><? } ?>
    </ul>
...
?>
	...
	<style>
	.s1 { background: url(images/DEFAULT/_icons/new/acc/acc_1.jpg) no-repeat; background-position: 0% 50%}
	.s2 { background: url(images/DEFAULT/_icons/new/acc/acc_2.jpg) no-repeat; background-position: 0% 50%}
	.s3 { background: url(images/DEFAULT/_icons/new/acc/acc_3.jpg) no-repeat; background-position: 0% 50%}
	.s4 { background: url(images/DEFAULT/_icons/new/acc/acc_4.jpg) no-repeat; background-position: 0% 50%}
	.s5 { background: url(images/DEFAULT/_icons/new/acc/acc_5.jpg) no-repeat; background-position: 0% 50%}
...
<iframe id="PreviewFrame" src="<?=DB_DOMAIN ?>index.php?dll=profile&pId=<?=$_SESSION['uid'] ?>" frameborder="0" style="width:650px; height:500px; border:3px dashed #cccccc; margin-top:10px;"></iframe>
...
<ul class="form" id="form4" style="display:visible">
	...
	<div class="menu_box_title1" onClick="toggleLayer('bb4');" style="cursor:pointer;"><span><img src="<?=DB_DOMAIN ?>images/DEFAULT/blank.gif" width="30" height="29" onClick="expandcontent(this,'s1')" class="menu_noexpand"></span><?=$GLOBALS['LANG_ACCOUNT']['8'] ?> <img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/add.png" align="absmiddle">  </div> 
...
<span class="<?php if($i % 2){ ?>search_display_off<? }else{ ?>search_display_on<? }  ?>" style="width:670px;">
			<li id="comment_<?=$comment['id'] ?>" <? if($comment['approved']=="no"){ ?> style="background:#FAE3EA" <? } ?>> 			
			  ...
			  <!-- Message Text -->
				<span> 
				  <a href="<?=DB_DOMAIN ?>index.php?dll=profile&pId=<?=$comment['fromid'] ?>"><img src="<?=$comment['image'] ?>" class="thumb-mail"></a> <?=$comment['comments'] ?><br><br>
