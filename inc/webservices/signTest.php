...
elseif ($action == "SIGNUP") {
    ...
    switch($SwitchValue) {
        ...
        case "username": {
            ...
            $errorMessage = $GLOBALS['LANG_REGISTER'][1];
    ...
    $em_m5z20131006 = trim(strip_tags($_POST['FieldValue87']));
    $em_5jb20131011 = trim(strip_tags($_POST['FieldValue92']));
    $em_1k820080113 = trim(strip_tags($_POST['FieldValue31']));
    $em_heh20080113 = trim(strip_tags($_POST['FieldValue32']));
    $em_s1620080113 = trim(strip_tags($_POST['FieldValue48']));
    $em_kjc20080113 = trim(strip_tags($_POST['FieldValue49']));
    $em_v9620131005 = trim(strip_tags($_POST['FieldValue86']));
    $year = trim(strip_tags($_POST['FieldValue23a']));
    $month = trim(strip_tags($_POST['FieldValue23b']));
    $date = trim(strip_tags($_POST['FieldValue23c']));
    $age = $year."-".$month."-".$date;
    ...
    $DB->Insert("UPDATE members_data SET uid= ( '".$userid."') ,postcode = '$postcode', age = '$age', headline='', country = '$country', location = '$location', description = '', gender = '$gender', em_m5z20131006='$em_m5z20131006', em_5jb20131011 = '$em_5jb20131011', em_1k820080113='$em_1k820080113',em_heh20080113='$em_heh20080113', em_s1620080113='$em_s1620080113',em_kjc20080113='em_kjc20080113', em_v9620131005='$em_v9620131005', em_85820081128='$em_85820081128'  WHERE uid= ( '".$userid."' ) LIMIT 1");
...
//    require_once("webservices_methods.php");
//    require_once("serviceUpload.php");
//    $UploadMax = 0;
//    while($UploadMax < 13){
//        // IF THE USER DOESNT HAVE AN ALBUM, CREATE ONE
//        //if(!isset($values['aid'])){ $values['aid']="new";}
//        if(is_array($Files["uploadFile0".$UploadMax]) && $Files["uploadFile0".$UploadMax]['type'] !="" ){ // error 4 = empty file
//
//                  $Status = UploadFileService($Files["uploadFile0".$UploadMax], $userid,$username, strip_tags($title), strip_tags($comments), 1, 'photo', $albumID,'no',$member_Package_id);
//
//        }
//        $UploadMax++;
//    }

    include_once("signUpUploadFile.php");
    $UploadMax = 0;
    while($UploadMax < 25){
    ...
    if($Status == "**complete"){
        $image_status = "Successfully uploaded.";
    }
    else{
        $image_status = $Status;
    }
    ...
    echo json_encode(array("message" => "SUCCESS", "method" => "SIGNUP", "status"=>"activateAccount", "image_status"=>$image_status));
...
function addToNetworkForFavorite($userID, $favID, $netID){
    ...
    if($result['found']  > 0){
        echo json_encode(array("message" => "FAIL", "method" => "ADD_TO_FAVORITE", "status"=>"This members is already is already a favorite.")); exit;
    }
    else{
...
function SendWinkMessage($sendfrom,$sendto){
    ...
    if($Total['countwink']==0){
        ...
        //$headers .= "CC: siddharth.mishra85@gmail.com\r\n";
