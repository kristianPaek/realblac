...
//print "FieldMulti $x $fieldId : $z<br>";

$y = $pageGET['Multi'.$x.$fieldId];
//print "Multi $x $fieldId : $y<br>";
...
if(($NETWORK_ID ==1) || ($NETWORK_ID ==3)){
	...
	$userData = $DB->Query("select gender, em_m5z20131006 from members_data where uid='". $_SESSION['uid'] ."' ");
	$otherType = "";
	$otherInterest = "";
...
fclose($fp);
...

    {
		...
		$DataArray[$Counter]['country'] 		= MakeCountry($Data['country']);					// MEMBERS COUNTRY
...
function MakeSavedSearched($data){
	...
	foreach($data as $ff => $key){
		...
		<form method="post" name="SaveSearch'.$i.'" action="'.DB_DOMAIN.'index.php?dll=search&view_page=1">';
