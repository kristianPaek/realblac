...
function GetProfileData($id, $selectedGroup="", $ShowTextGroupsOnly=0){
	...
	if($ShowTextGroupsOnly ==2){
							...
							if(trim($DataArray[$field['fName']])!=''){  
								...
								$ReturnString .= nl2br($DataArray[$field['fName']])."</div>"; 
							} else {
								$ReturnString .= '';
							}
							...
							// DISPLAY THE OUTPUT
							if($DataArray[$field['fName']] !="" && $DataArray[$field['fName']] !="0"){  
								$ReturnString .= '&nbsp;&nbsp;&nbsp;<div style="width:305px; float:left; min-height:20px;">'.$Caption['caption'].':<span>&nbsp;<strong>';
