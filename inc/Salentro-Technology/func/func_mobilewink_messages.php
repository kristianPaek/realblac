			...
			*/
			...
			case "add": {
				...
				foreach($ToArray as $sending_to){
					...
					$usedimagespace = $DB->Row("SELECT count(uid) AS space FROM messages WHERE uid= ( '".$_SESSION['uid']."' ) AND type = 'wink' AND maildate='".DATE_NOW."'");	
					...
					if( ($usedimagespace['space'] >= $_SESSION['pack_messages']) && D_FREE =="no"){
							...
							if($blocked['total'] ==0){
								...
								echo '@@@'."INSERT INTO `messages` ( `uid` , `mailnum` , `mail2id` , `mailstatus` , `maildate` , `mailtime` , `mail_subject` , `mail_message` , `mail_displayalert`, my_box, to_box )
								VALUES ('".$_SESSION['uid']."', NULL , '".$TUID."', 'unread', '".DATE_NOW."', '".TIME_NOW."', '".$MessageSubject."', '".$MessageData.$CardCode."', '1', 'sent', 'inbox')";die;
