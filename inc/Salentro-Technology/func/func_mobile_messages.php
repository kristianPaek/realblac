				...
				foreach($ToArray as $sending_to){
					...
					if( ($usedimagespace['space'] >= $_SESSION['pack_messages']) && D_FREE =="no"){
							...
							if($blocked['total'] ==0){
                                ...
                                $MessageData =GetMsgNote($MessageData,$_SESSION['uid']).$MessageData1;
								...
								if(DoEmailSMS($TUID,5,'email_msg',substr($MessageData,0,30))==2){
									header("location: ".DB_DOMAIN."mobile.php?dll=mobilemessages&sub=inbox");
								}
							}else{
