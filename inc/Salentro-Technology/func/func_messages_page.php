...
function DisplayMessages($id,$box=1, $orderBy="maildate", $Start="0", $Stop="10",$errMessage=""){
...

    {
			...
			if($msg['maildate'] == DATE_NOW){
				...
				$ReturnMessageArray[$NumFields]['date'] = $GLOBALS['LANG_MESSAGES']['today'];
...
function GetMsgChainData($id){
	...
	$msg = $DB->Row("SELECT messages.mail_message, messages.maildate, mailtime,members.username 

	FROM messages
	INNER JOIN members ON ( members.id = messages.uid )
	WHERE  messages.mailnum= ( '".$id."' ) LIMIT 1");

	

	## create replay chain
        $chaindata="\n\n on ".$msg['maildate'] ." ".date("h:i:s A", strtotime($msg['mailtime'])) .", ". $msg['username'] ." wrote: \n\n" .$msg['mail_message'];
...
function GetMsgFromId($id){
    ...
    $msg = $DB->Row("SELECT messages.mail_message, messages.maildate, mailtime
