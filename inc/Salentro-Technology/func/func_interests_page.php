...
function DisplayCats(){
	...
	## define variables
	$Counter= 1;	$DataArray = array(); $MODdata['page'] ='interests'; $MODdata['sub'] ='view'; $MODdata['type'] ='system';
	
	$result = $DB->Query("SELECT * FROM `interests_cats`");
	while( $Data = $DB->NextRow($result) )
	{
		...
		$result1 = $DB->Row("SELECT count(uid) AS totalc FROM groups WHERE cat_id ='".$Data['id']."'".$ExtraString);
		$result2 = $DB->Row("SELECT name, id, updated FROM groups WHERE cat_id ='".$Data['id']."' ORDER BY id DESC LIMIT 1");
...
function DisplayGroups($pageGET, $getData="", $This_Page, $edit=false){
	...
	## define variables
	$Counter=1;	$DataArray = array(); $MODdata['page'] ='interests'; $MODdata['sub'] ='show'; $MODdata['type'] ='system';
	...
	$SQL = "SELECT members.username, files.bigimage, files.approved, files.adult_content, files.type, groups.approved AS ThisApproved, groups.* FROM `groups` 
...
//print $SQL;
	...
	while( $Data = $DB->NextRow($result) )
	{
		...
		$result1 = $DB->Row("SELECT count(uid) AS totalc FROM `interests_members` WHERE group_id ='".$Data['id']."'");
...
function DisplayNetwork($id){
	...
	INNER JOIN interests_members ON ( interests_members.uid = members_data.uid )
	LEFT JOIN files ON ( files.uid = members_data.uid ) 
	WHERE  interests_members.group_id= ( '".$id."' )  GROUP BY members.id");
...
function GroupDetails($id){
	...
	$result = $DB->Row("SELECT name FROM `interests_cats` WHERE id='".$id."' LIMIT 1");
...
function GroupInnerDetails($id){
	...
	$result = $DB->Row("SELECT files.bigimage, files.approved, files.adult_content, files.type, groups.approved AS ThisApproved, members.username, groups.*, interests_cats.name 
	FROM `groups` 
	INNER JOIN interests_cats ON (groups.cat_id = interests_cats.id)
	LEFT JOIN files ON (groups.photo = files.bigimage ) 
	...
	$Joined = $DB->Row("SELECT count(uid) AS total FROM interests_members WHERE group_id ='".$result['id']."' AND uid='".$_SESSION['uid']."' LIMIT 1");
	if($Joined['total'] ==0){
		$result['joined'] ="no";
	}else{
		$result['joined'] ="yes";
...
function DisplayGroupsTopicsTotal($id){
	...
	$result = $DB->Row("SELECT count(id) AS total FROM interests_topics WHERE interests_id='".$tid."' LIMIT 1");
...
function TopicDetails($id){
	...
	$result = $DB->Row("SELECT title, comments FROM `interests_topics` WHERE id='".$tid."' LIMIT 1");
