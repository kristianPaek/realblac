...
$result = $DB->Row("SELECT count(id) AS total FROM members_network WHERE uid='".$_REQUEST['pId']."' AND to_uid='".$_SESSION['uid']."' AND type=3");
...
if($result['total'] ==1){
	...
	die("SORRY YOU HAVE BLOCKED CHATTING WITH THIS MEMBER");

}

$result = $DB->Row("SELECT count(id) AS total FROM members WHERE id='".$_REQUEST['pId']."' and visible = 'no' LIMIT 1");

if($result['total'] ==1){

	// THIS USER HAS BLOCKED

	die("SORRY THIS MEMBER IS NOT AVAILABLE TO CHAT RIGHT NOW");
	...
	return string;
				...
				if(sendChatClosed)
				{
					...
					if(chat_toid != chat_fromid)
					{
						if (closeReq.readyState == 4 || closeReq.readyState == 0 ) {
							...
							closeReq.open("POST", 'inc/imSend.php?chat=1&chat_fromid=' + chat_fromid + '&chat_toid=' + chat_toid + '&last=' + lastMessage + '&read=1&endchat=1', true);
							closeReq.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
							//closeReq.onreadystatechange = handleSendChat; 
			...
			function sendChatText() {
				...
				if (sendReq.readyState == 4 || sendReq.readyState == 0) {
					...
					sendReq.open("POST", 'inc/imSend.php?message='+unescapeHTML(ed.getContent())+'&chat=1&chat_fromid=' + chat_fromid + '&chat_toid=' + chat_toid + '&last=' + lastMessage, true);
