...
if ($row=db_simpleSelect(0,$Tu,$dbUserSheme['moderator'][1].','.$dbUserId,'','','',$dbUserId)){
...
$title=$sitename;
echo load_header($layout[1]["contents"]); echo ParseTpl(makeUp('main_forums'));
}
else{
$errorMSG=$l_stillNoForums; $l_returntoforums=''; $correctErr='';
echo load_header($layout[1]["contents"]); echo ParseTpl(makeUp('main_warning'));
}
?>