...
function sendwink(sendfrom,sendto){
	document.getElementById('winkp').innerHTML = '<span style="font-size:10px; color: green;">sending...</span>';
	eMeetingDo('inc/ajax/_actions.php?action=SendWink&sendfrom='+sendfrom+'&sendto='+sendto,"winkp");
}
	...
	{				
		...
		Ajax.mRequest.onreadystatechange = function() {
			if(Ajax.mRequest.readyState == 4 && Ajax.mRequest.status == 200){
				if(div=='winkp') {
					var rt = '<span style="font-size:14px; color: green;">'+Ajax.mRequest.responseText+'</span>';
					obj.innerHTML = rt;
				}
				else { 
...
function rotateimg(fileid)
{
    eMeetingDo('inc/ajax/_actions.php?action=RotateImg&fileid='+fileid,"response_gallery");
