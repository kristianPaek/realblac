<?php
$LANG_ = array(
"_language" => "English",
"_charset" => "UTF-8", 
);

$LANG_COMMON = array(

	"0" => "Page %1 of %2 ",
	"1" => "Forgotten Password",
	"2" => "All Members",
	"3" => "My Friends",
	"4" => "My Favorites",
	"5" => "My Photos",
	"6" => "My Classifieds",
	"7" => "My Blogs",
	"8" => "My Comments",
	"8a" => "My Events",
	"9"  => "Send Message",
	"10" => "Send Wink",
	"11" => "View Profile",
	"12" => "Add to Favorites",
	"13" => "Send Friend Request",
	"14" => "Block Member",
	"15" => "  Search Members  |",
	"16" => "Add as profile partner",
	"17" => "Featured Events",
	"18" => "Featured Members",
	"19" => "Featured Groups",
	"20" => "Featured Videos",	
	"21" => "Latest News",
	"22" => "Press Releases",
	"23" => "My Photos",
	"24" => "Recent Member Activity",
	"25" => "Never Updated",
	"26" => "%1 seconds ago",
	"27" => "%1 minutes ago",
	"28" => "%1 days ago",
	"29" => "%1 weeks ago",
	"30" => "%1 months ago",
	"31" => "%1 years ago",
	"32" => "%1 hours ago",
	"33" => "Message Sent",
	"34" => "Block Messages From",
	"35" => "If you would like to block a particular gender from contacting you highlight them in the list above.",
	"36" => "My Messages",
	"37" => "My Groups",
	"38" => "Play Video",
	"39" => "New Message(s)",
	"40" => "New Friends",
	"41" => "New Comments",
	"42" => "Send Wink",
);

$LANG_WELCOME = array(

	"_join" => "Not yet a member?",
	"_join1" => "Click Signup button below to make your profile on our site. Its free to join so what are you waiting for?<br /><br />",
	"_join2" => "Join Now",

	"_ct1" => "Join Us For Free",
	"_ct2" => "Start Chatting",
	"_ct3" => "Meet Great Singles!",
	
	"_cb1" => "Create your free profile today, it only takes a minute!",
	"_cb2" => "We offer many different ways for you to know our members!",
	"_cb3" => "Find that special person now, Anything is possible!",

	"1" => "Latest Website Blog",
	"2" => "Read The Full Blog",
	"3" => "Three Steps to Success!",
	"4" => "1. Register Now",
	"5" => "2. Create a profile",
	"6" => "3. Find true love",
	"7" => "Its easy to get started,  signup now and create your free profile, within minutes you could be meeting the right person for you!",
	"8" => "Quick Member Search",
	"9" => "Take a look at some of our other members.",

);
$LANG_ERROR = array(
	"_complete" => "Your account has been successfully updated. If you have updated your profile, approval is needed by member services before it goes live. Thanks for your patience.",
	"_incomplete" => "Please complete all the fields",
	"_notaccepted" => "File type not accepted.",
	"_error" => "There was an error with the data you entered. Please check and try again.",
	"_offline1" => "Website Temporary Offline for Maintenance",
	"_offline2" => "We are currently updating our website and will be back online shortly.", 
	"_offline3" => "We apologize for any inconvenience and thank you for your patience.",  
	"_noResults" => "Sorry there were no results found!",
	"_inviteMsg" => "I found this new dating site and I thought you would be interested in checking it out, the link is",
	"_welcomeMsg" => "Welcome to RealBlackLove the award winning #1 dating app for relationship focused black singles. Your profile is Limited status and pending review. Once approved it will be visibile to members. As a limited member you can upload five photos, send unlimited winks, send or reply to one email daily for FREE. Want full access? Upgrade to a VIP membership now. Happy dating!",
	"_invalid" => "Your %s address is invalid",
	"_invalidCode" => "Verification Code you entered was invalid. Please check and try again.",
	"_adultWarning" => "Please select yes if this file contains adult content.",
	"_waitingApproval" => "This member is new and just submitted his/her profile for approval. Please check back soon.",

	"_AlertQuickMsg" => "Enter your message below to contact %s",
 	"_AlertQuickWink" => "Send a wink to <b>%s</b>.<br /> Upgrade to VIP for more winks. ",
	"_userBlocked" => "We are sorry but you cannot communicate with this user as they have blocked you.",

	"_videoIntro" => "<p><b>Record your personal introduction!</b></p><p>Plugin in your webcam and microphone and get started with your own profile introduction. </p><p>When your ready, click on the record button, record your audio/video for up to 5 minutes and then press the stop button when your done! Don't forget to preview your video and then save it before you exit.</p>",
	"_emailSent" => "Thank You, your email has been sent.",

	"_noT&C" => "Please check the box to accept the website terms and conditions.",
	"_eventWho" => "Would you like this event to be displayed to your friends only or to everyone on the website?",
	"_fileE1" => "The file appears to be corrupt.",
	"_fileE2" => "We were unable to generate any photo data from this file. Please try again with a different photo.",
	"_fileE3" => "File is empty. Please upload something more substantial.",
	"_fileE4" => "Bad MIME type submitted by your browser.",
	"_fileE5" => "Bad file extension.",
	"_fileE6" => "Im sorry but we do not support [%s] file types, please upload a JPEG image file.",
	"_fileE7" => "Bad temp file. Try renaming the file and uploading again.",
	"_fileE8" => "Bad file. Try again, or try recreating it.",
	"_fileE9" => "The file is blank.",

	//
	"_noAdultAccess" => "Sorry you dont have permissions to view adult files.",

	"_t10" => "",
	"_t11" => "Top Rated Members",

	"_t12" => "Save This Search",
	"_t13" => "My Saved Searches",
);
$LANG_UPGRADE = array(	
	"0" => "Thank You",
	"_complete" => "Upgrade Complete, Congratulations. You now have VIP access to the site.",
	"2" => "Logout Now",
	"_cancel" => "Order Canceled",
	"4" => "Upgrade Canceled",
	"6" => "Upgrade Error",
);
$LANG_BODY = array(

"_yold" 			=> "years old",

// A
"_account" 			=> "Account",
"_all" 			=> "All",
"_add" 			=> "Add",
"_addComment" 	=> "Add Comment",
"_approve" 		=> "Approve",
	"_approved" 		=> "Approved",
"_article" 		=> "Press Release",
	"_articles" 	=> "Press Releases",
"_adultContent" => "Adult Content",
"_accountOverview" => "Overview",
"_advert" 			=> "Adverts",
"_affiliate" 		=> "Partner",
"_age" 		=> "Age",
"_advanced" 		=> "Advanced",
"_album" 		=> "Photo",
	"_albums" 		=> "Photo",
"_address" 		=> "Address",
"_attending" 			=> "Attending",
//B
"_banners" 		=> "Banners",
"_blog" 		=> "Blog",
"_blockList" 		=> "Members I've Blocked",
//C
"_category" 	=> "Category",
"_created" 		=> "Member joined",
"_createNew" 	=> "Create New",
"_createdBy" 	=> "Created By",
"_change" 		=> "Change",
"_comments" 	=> "Comments",
"_country" 		=> "Country",
"_city" 		=> "Town / City",
"_cancel" 		=> "Cancel",
"_current" 		=> "Current",
"_correct" 		=> "Correct",
"_credits" 		=> "Credits",
//D
"_delete" 		=> "Delete",
	"_deleted" 		=> "Deleted",
"_deselectAll" 	=> "Deselect",
"_date" 		=> "",
"_displayPhoto" => "",
"_day" 			=> "Day",
"_description" 	=> "Description",
//E
"_edit" 		=> "Edit",
"_error" 		=> "Error",
"_email" 		=> "Email",
"_event" 		=> "Event",
	"_events" 		=> "Events",
"_everyone"		=> "Everyone",
"_expires" 		=> "Expires",

//F
"_fax" 			=> "Fax",
"_faq" 			=> "FAQ",
"_friend" 		=> "Friend",
	"_friends" 		=> "Friends",
	"_friendsList" 		=> "Friends List",
"_featured" 	=> "Featured",
"_favorites" 	=> "Favorites",
"_file" 		=> "Upload",
	"_files" 		=> "Uploads",
"_fileType" 		=> "Type",
"_forgotten" 		=> "Forgotten",

//G
"_groups" 		=> "Groups",
"_gender" 		=> "Gender",
//H
"_hightlighted" => "Highlighted",
"_hotList" 		=> "Favorites",
//I
"_information" 	=> "Details",
"_itsFree" 		=> "It's Free",
"_location"		=> "City",
//J
"_joinToday" 	=> "Joined Today",
"_joined" 		=> "Joined",
//k
"_keyword" 		=> "Keyword",
//L
"_latest" 		=> "Latest",
"_login" 		=> "Login",
"_lastLogin" 	=> "Last Login",
"_logout" 		=> "Logout",
"_link" 		=> "Link",
//M
"_manage" 			=> "Manage",
"_member" 			=> "Member",
	"_members" 			=> "Members",
//"_message" 			=> "",
	"_messages" 			=> "Messages",
	"_wmessages" 			=> "Your Winks",
"_month" 			=> "Month",
"_my" 				=> "My",
"_mile" 			=> "Mile Radius",
"_music" 			=> "Music",
"_membership" 		=> "Membership",
//N
"_no" 			=> "No",
"_next" 		=> "Next",
"_name" 		=> "Name",
"_namef" 		=> "First Name",
"_namel" 		=> "Last Name",
"_nameb" 		=> "Company Name",
"_now" 			=> "Now",
"_new" 			=> "New",
//O
"_options" 		=> "Options",
"_of" 		=> "of",
//P
"_photo" 		=> "Photo",
	"_photos" 		=> "Photos",
"_play" 		=> "Play",
"_previous" 	=> "Previous",
"_page" 		=> "Page",
"_payments" 	=> "Payments",
"_privacy" 		=> "Privacy",
//0
"_online" 		=> "Online",
"_onlineNow" 	=> "Online Now",
//V
"_view" 		=> "View",
	"_views" 		=> "Views",
"_visitors" 	=> "Visitors",
"_viewAll" 		=> "",
"_verification" => "Verification Code", 
"_video" 		=> "Video",
	"_videos" 		=> "Videos",
"_visibility" 		=> "Visibility",
//q
"_question" 	=> "Question",
	"_questions" 	=> "Questions",
"_quiz" 	=> "My LoveQuizes",
"_quiz1" 	=> "LoveQuiz",
//U
"_username" 	=> "Username",
"_updateAccount" => "Update Profile",
"_updated" 		=> "PRESS OK TO CONFIRM THIS ACTION.",
	 "_update" 		=> "Congrats. Its Done!",
//r
"_results" 		=> "Members",
"_recurring" 	=> "Recurring",
"_remove" 		=> "Remove",
"_rating" 		=> "Rating",
"_rememberMe" 	=> "Remember Me",
"_register" 	=> "All done. Sign me up!",
"_read" 		=> "Viewed",
"_reply" 		=> "Send a Reply",
"_removed" 		=> "Removed",
//S
"_speadWord" 	=> "Spread the word",
"_save" 		=> "Save",
"_sent" 		=> "Sent",
"_settings" 	=> "Settings",
"_search" 		=> "Search",
"_searchQ" 		=> "Quick Search",
"_select" 		=> "Select",
"_selectAll" 	=> "Select All",
"_submit" 		=> "Submit",
"_sort" 		=> "Sort By",
"_send" 		=> "Send",
"_slideshow" 	=> "",
	"_sort0" 	=> "Basic View",
	"_sort1" 	=> "Gallery View",
	"_sort2" 	=> "Detailed View",
	"_sort3" 	=> "Recently Added",
	"_sort4" 	=> "Recently Updated",
	"_sort5" 	=> "Most Popular",
	"_sort6" 	=> "Show %s Per Page",
"_street" 		=> "PO Box",
"_status" 		=> "Status",
"_subject" 		=> "Subject",
"_song" 		=> "Profile Song",
//P
"_password" 	=> "Password",	
"_province" 	=> "State",
"_phone" 		=> "Phone",
"_profile" 		=> "Profile",
"_partners" 		=> "Partner",
//T
"_thankyou" 	=> "Thank You",
"_topic" 		=> "Topic",
"_top10" 		=> "Top 10",
"_tags" 		=> "Tags",
"_title" 		=> "Title",
"_time" 		=> "Time",
"_today" 		=> "Today",
//Y
"_yes" 			=> "Yes",	
"_year" 		=> "Year",
//W
"_watch" 		=> "Watch",	
"_website" 		=> "Website URL",
"_wink" 		=> "Wink", 
"_within" 		=> "Within a",	
"_withPics" 	=> "With Photos",	
"_zipcode" 		=> "Zip code",
// MONTHS
"_monday" => "Monday",
"_tuesday" => "Tuesday",
"_wednesday" => "Wednesday",
"_thursday" => "Thursday",
"_friday" => "Friday",
"_saturday" => "Saturday",
"_sunday" => "Sunday",
// DAYS
"_january" => "January",
"_febuary" => "February",
"_march" => "March",
"_april" => "April",
"_may" => "May",
"_june" => "June",
"_july" => "July",
"_august" => "August",
"_september" => "September",
"_october" => "October",
"_november" => "November",
"_december" => "December",

// ALERTS
"_alert1" 		=> "My Alerts",	
"_atTitle" 		=> "",
"_atSub" 		=> "",
"_atNo" 		=> "Dont Attach My Photos",


// MODERATOR TEXT 
"_mod1" 		=> "Moderator Status",	
"_mod2" 		=> "Live Approval",
"_mod3" 		=> "Live Edit",
"_mod4" 		=> "Live Delete",

"_pChat" => "Private Chat",
"_pLink" => "Profile Link",
"_details" => "Details",

"_pSmsg" => " Headline:",

"_skype1" => "Enable Skype Calls",
"_skype2" => "Simply enter your skype username into the box above and we will place a call me button onto your profile, members can then chat to you.",

"_seconds" => "seconds",
"_minutes" => "minutes",
"_hours" => "hours",
"_months" => "months",
"_today" => "today",
"_ago" => "ago",
"_about" => "about",
"_days" => "days",

"_msgSent" => "Message Sent",


 "_home1" => "I am a",
 "_home2" => "Seeking a",

"_star1" => "Capricorn",
"_star2" => "Sagittarius",
"_star3" => "Scorpio",
"_star4" => "Libra",
"_star5" => "Virgo",
"_star6" => "Leo",
"_star7" => "Cancer",
"_star8" => "Gemini",
"_star9" => "Taurus",
"_star10" => "Aries",
"_star11" => "Pisces",
"_star12" => "Aquarius",

//

"_CompleteProfile" => "Complete your profile",
"_menue1" => "Profiles I Viewed",

"_menue2" => "Who is around the corner.",
"_menue3" => "Quick Member Search",
"_menue4" => "Keyword Search",
"_menue5" => "Birthdays Today",
"_menue6" => "Joined in the last",
"_menue7" => "Select Period",
"_menue8" => "Profile Headline",
"_menue9" => "Profile Description",
"_menue10" => "Last Online",
"_menue11" => "Unapproved Members",
"_weeks" => "weeks",

 "_fpost" => "Make Post",

// IM CHAT
"_imchat1" => "Chat Request From",
"_imchat2" => "",
"_imchat3" => "Yes",
"_imchat4" => "No",

);

$lang_global_options = array(
	"1" => "Member Communication",
	"2" => "Print this page",
	"3" => "Email this page",
	"4" => "Post a comment",
	"5" => "Contact Member",
	"6" => "Add to Favorites",
	"7" => "Export to PDF",
	"8" => "Leave This Group",
	"9" => "Join This Group",
	"10" => "Attend This Event",
	"11" => "Remove Attending Status", 
	"12" => "",
	"13" => "Online Now",
	"14" => "Please Do Not Disturb",
	"15" => "My Settings",
	"16" => "Sign Off",
	"17" => "",
	"18" => "Spread the word",
	"19" => "Members Attending",
	"20" => "Recently Visited",
	"21" => "Recently Viewed",	
	"22" => "Copy Page Link", 
 	"23" => "Left a comment for ",
	"24" => "commented on the video ",
	"25" => "Left a comment on an album file by ",
	"26" => "Left a comment on a blog by ",
	"27" => "Left a comment on ",
	"28" => "Logged in ",
	"29" => "Updated their profile ",
	"30" => "Added a new photo ",
	"31" => "Changed their display photo ",
	"32" => "Is now attending ",
	"33" => "Added a new group ",
	"34" => "Changes their status to ",
	"35" => "Browse all categories",
	"36" => "Give it a Rating",
	"37" => "",
	"38" => "You have not created any yet! Click here to get started.",
	"39" => "Members can Post Topics",
	"40" => "Browse by Category",
 	"41" => "Events Calendar",
	"42" => "Click here to view your current video recordings",
	"43" => "Click here to record a new video",
	"44" => "Recent Member Updates",
	"45" => "Recent Forum Topics", 
);

$lang_main_menu = array(
"http://www.realblacklove.com/meet/" => "Home",
"http://www.realblacklove.com/meet/index.php?dll=settings" => "Settings",
"http://www.realblacklove.com/meet/index.php?dll=messages&sub=inbox" => "Inbox",

"http://www.realblacklove.com/meet/index.php?dll=favorites" => "Favs",

"http://www.realblacklove.com/meet/index.php?dll=overview&sub=viewed" => "Viewed Me",

"http://www.realblacklove.com/meet/index.php?dll=messages&sub=wink" => "Winks",

"http://www.realblacklove.com/meet/index.php?dll=account&sub=&dll=account&sub=view" => "My Profile",

"http://www.realblacklove.com/meet/index.php?dll=account&sub=edit" => "Edit Profile",

"http://www.realblacklove.com/meet/index.php?dll=gallery&sub=albums" => "Photos",

"http://www.realblacklove.com/meet/index.php?dll=search&sub=advanced" => "Search",

"http://www.realblacklove.com/meet/index.php?dll=forum" => "Forums",

"http://www.realblacklove.com/meet/index.php?dll=search&view_page=1" => "Members Online",

"http://www.realblacklove.com/meet/index.php?dll=blog&sub=search" => "Blogs",

"http://www.realblacklove.com/meet/index.php?dll=blog&sub=add" => "New Blog",

"http://www.realblacklove.com/meet/index.php?dll=subscribe" => "Upgrade",

"http://www.realblacklove.com/meet/index.php?dll=Affiliate" => "Partner",

"http://www.realblacklove.com/meet/index.php?dll=logout" => "Logout",

); 
$lang_main_menu_sub = array( 

); 

$lang_main_footer = array( 

);
/////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////

$LANG_ACCOUNT_MENU = array(
 
	"" => "Profile Options",
		"_?" => "",
	"&dll=account&sub=view" => "View My Profile",
		"&dll=account&sub=view_?" => "View your profile as other members will see it.",
	"&dll=gallery" => "My Photos",
		"&dll=gallery_?" => "Click here to view and manage your photos.",
	"edit" => "Edit Profile",
		"edit_?" => "Updated profiles will not go live until approved by our member services team. Please submit only one update at a time. <b>Warning</b>: posting offensive material, email addresses, websites, personal information, etc. are violations of site policy. Doing so jeopardizes your safety and may also lead to the suspension/termination of your account.",
	"design" => "Profile Designer",
		"design_?" => "Use the options below to customize your profile.",
	"video" => "My Introduction",
		"video_?" => "Let your personality shine! Record a video or audio introduction.",
	"comments" => "My Comments",
		"comments_?" => "View, delete your website comments here.",
	"view" => "",
		"view_?" => "",
);

$LANG_MESSAGES_MENU = array(

	"" => "My Messages",
		"_?" => "",
	"inbox" => "My Inbox",
		"inbox_?" => "View and manage your inbox messages here.",
	"wink" => "",
		"wink_?" => "",
	"sent" => "Sent Mail",
		"sent_?" => "View your sent messages here.",
	"trash" => "Trash Can",
		"trash_?" => "Deleted messages are put in your trash can. Manage them here.",
	"create" => "Send Message",		
		"create_?" => "Create a new message to send to other members here.",
	"read" => "Read Message",		
		"read_?" => "",
);

$LANG_GALLERY_MENU = array(

	"search" => "",	
		"search_?" => "",
	"albums" => "View My Photos",
		"albums_?" => "View and manage your photos here.",
	"create" => "",
		"create_?" => "Create a new photo album and upload your images.",
	"upload" => "Add Photos",	
		"upload_?" => "Upload a new photo to your album here.",
	"display" => "Change Main Photo",
		"display_?" => "Choose a photo that stands out as your main photo.",
	"manage" => "",	
		"manage_?" => "Click any of your photo album files below to view its contents.",
	"edit" => "",	
		"edit_?" => "Use the options below to edit your photo details.",

);

$LANG_MATCH_MENU = array(

	"test" => "LoveQuiz",
		"test_?" => "Manage your LoveQuiz or review LoveQuiz results.",

	"addquiz" => "Add New LoveQuiz",
		"addquiz_?" => "",

	"results" => "LoveQuiz Results",
		"results_?" => "",

	"add" => "Manage Questions",			
		"add_?" => "",

	"taken" => "LoveQuizes Taken",			
		"taken_?" => "View and manage LoveQuizzes you have taken here.",
);

$LANG_EVENTS_MENU = array(
	"events" => "View All Events",
		"events_?" => "View all of the upcomming events here.",	
	"add" => "Add Event",
		"add_?" => "Heard about a new event? Why not tell the world and find out which other members are also attending.",	
	"manage" => "My Events",		
		"manage_?" => "Manage your existing events here.",
	"search" => "Search Events",		
		"search_?" => "",
);
$LANG_GROUPS_MENU = array(
	"search" => "View All Groups",
		"search_?" => "View all the website groups here.",
	"add" => " Create Group",
	"view" => "View Groups",
		"view_?" => "Use the options below to browse through the member groups.",
		"add_?" => "Why not create a new group and allow members to share your interests.",
	"manage" => " My Groups",		
		"manage_?" => "Manage your existing groups here.",
	"show" => "",		
		"show_?" => "",
);

$LANG_CLASSADS_MENU = array(

	"search" => "Classified Ads",
		"search_?" => "View all the Classified ads created by our members.",
	"manage" => "View My Ads",
		"search&fcid=0_?" => "View all the of Classified ads you have created here.",
	"add" => "Create Advert",
		"add_?" => "Create a new Classified ad here.",
	"view" => "",		
		"view_?" => "",
);
$LANG_MUSIC_MENU = array(
	"search" => "All Music Files",
		"search_?" => "View all of the music files added by our members here.",
	"view" => "My Music",
		"view_?" => "View and manage music files you have uploaded here.",
	"" => "",
		"_?" => "",
);

$LANG_VIDEO_MENU = array(
	"search" => "View All Videos",
		"search_?" => "View all of the our member videos here.",
	"myvideos" => "My Videos",
		"myvideos_?" => "Listed below are all the videos you have added to your account.",

	"view" => "",
		"view_?" => "",
);

$LANG_INVITE_MENU = array(
	"&dll=invite" => "Invite Friends",	
		"&dll=invite_?" => "Send an invitation to your friends using the options here.",
);
$LANG_1GAME_MENU = array(
	"search" => "View All Games",
		"search_?" => "View and play any of the games listed on our website here.",
	"top" => "Leaderboard",
		"top_?" => "Check our the leaderboard of the top scores.",
	"play" => "Play Games",
		"play_?" => "",
);
$LANG_SETTINGS_MENU = array(

	"privacy" => "Privacy Settings",
		"privacy_?" => "Manage your account privacy settings here",
	"password" => "Change Password",
		"password_?" => "Update your account login password here.",
	"cancel" => "Cancel Account",
		"cancel_?" => "Saying goodbye? Contact us at cancel@realblacklove.com from the email address you registered to the site. Please include your username and the reason for account cancellation.  Please allow 3-4 business days for account cancellation.",
	"sms" => "Account Alerts",	
		"sms_?" => "Manage and update your SMS / Text message settings here.",
	"alerts" => "Email Alerts",			
		"alerts_?" => "Manage and update email alert settings here.",
	"settings" => "",
		"settings_?" => "",

);
$LANG_BLOG_MENU = array(
	"search" => "Member Blogs",
		"search_?" => "Check the latest member blogs here by our members.",
	"view" => "View My Blogs",
		"view_?" => "View and manage your blog entries here.",
	"add" => "Create New Blog",
		"add_?" => "Create a new blog and tell the community about what's happening with you.",
);
$LANG_LINKS_MENU = array(

	"links" => "Our Partners",
		"links_?" => "Interested in becoming a partner of RealBlackLove.com?  Please contact us today to have your business listed here.",

);
// search page
$LANG_SEARCH = array(

	"search" => "View All members",
		"search_?" => "",

	"search&friendid=2" => "My Friends",
		"search&friendid=2_?" => "",

	"search&friendid=1" => "My Favorites",
		"search&friendid=1_?" => "My Favorites",

	"search&friendid=2&friend_type=3" => "Blocked Members",
		"search&friendid=2&friend_type=3_?" => "",
);

$LANG_CHAT_MENU = array(

	"chatroom" => "Chat Room",
		"chatroom_?" => "",
	"im" => "Instant Messenger",
		"im_?" => "",
	"wink" => "Send Wink",
		"wink_?" => "",
	"profile" => "View Profiles",
		"profile_?" => "",
	"forum" => "View Forum",
		"forum_?" => "",
);

// contact PAGE
$LANG_CONTACT_MENU = array(

	"" => "Contact Us",	
		"_?" => "Let's get Social! <br /><b>Twitter @4RealBlackLove<br />Facebook.com/4RealBlackLove</b><br /><br /><br /><b>We are dedicated to offering the highest level of support to you by answering any inquiry you may have about our website or services. Email us using the form below.<br />",


);
// sitemap
$LANG_SITEMAP_MENU = array(

	"" => "Site Map",	
		"_?" => "",

);
$LANG_UPGRADE_MENU = array(

	"" => "My Membership",	
		"_?" => "<i><center>Limited members can upload five photos, send unlimited winks and send or reply to one email daily for FREE. </i><br /><br /><b>VIP MEMBER UPGRADE AVAILABLE: </b><BR />Unlimited Emails & Winks. <br />Upload up to 25 photos.<br /><br />
<b>- Website Launch Special -<br />50% off 6 month upgrades<br /> Regular price is $39.95<br /> Today you invest only $19.95<br /><hr>USE COUPON CODE: BlackLove50 <br />Offer Expires Soon! </b></center><br />
<hr>
<i>Visa and MasterCard are our only preferred methods for transactions. </i>",

	"matrix" => "",	
		"matrix_?" => "Listed below are the current upgrade packages and their access features.",
);

$LANG_AFFILIATE_MENU = array(
 
	"" => "Partner Network",
		"_?" => "",
	"login" => "Partner Login",
		"login_?" => "",
	"join" => "Become our Partner Today!",
		"join_?" => "",
	"summary" => "Account Summary",	
		"summary_?" => "",
	"banners" => "Banners",			
		"banners_?" => "",
	"edit" => "My Account",	
		"edit_?" => "",
	"payment" => "Payment History",			
		"payment_?" => "",
);

$LANG_FORUM_MENU = array(
	"" => "Ask a Matchmaker",	
		"_?" => "",
);
// articles
$LANG_ARTICLES_MENU = array(
	"" => "",	
		"_?" => "Listed below are RealBlackLove.com's official press releases.",

	"view" => "",	
		"view_?" => "",
);

// subscribe
$lang_subscribe = array(

	"" => "Upgrade My Membership",	
		"_?" => "Use the options below to upgrade your membership",
	"a1" => "Upgrade My Account",
	"a3" => "Upgrade Your Account Today!",
	"a4" => "Ill upgrade later",
	"a5" => "The benefits of a VIP member!",
	"a6" => "Click a package to view the features",
	"a7" => "Upgrade Packages",		
	"a8" => "Payment Choices",	
	"a9" => "Payment Methods",	
	"a10" => "In order for you to benefit from the reduced rates, Passes are renewed automatically on a monthly basis once your initial subscription expires. You may of course cancel the automatic renewal at any time by contacting us.",
	"a11" => "We Accept",	
	"a12" => "Payment Methods",
	"a13" => "Select payment method",
	"a14" => "Upgrade Now",		
	"15" => "Current Membership",
	"16" => "Bank / Wire Payment",
	"17" => "What you need todo",		
	"18" => "Please ask your bank to transfer",
	"19" => "to us using the details listed below. Your account will be created and ready to use as soon as we receive your payment.",
	"20" => "Extra SMS Credits",
    "21" => "Apply Coupon",
);

 
$WINK_MESSAGE_ARRAY = array(
	"0" => "Just wanted to say hello. Nice Profile!",
	"1" => "I would love to see a photo of you.",
	"2" => "Great profile. Let's talk!",
	"3" => "Here is a kiss for you.",
	"4" => "Hi. How are you?",
	"5" => "I am interested in you.",
	"6" => "I Like You!",
	"7" => "I think you are very good looking.",
	"8" => "Seems as if we have plenty in common.",
	"9" => "Busy this weekend?",
        "10" => "I think I am in love with you.",
        "11" => "Will you marry me?",
        "12" => "Keep smiling, your smile is infectious.",
        "13" => "Thank you. But I am not interested.",
);

$lang_overview_page = array(
		
	"a7" => "Record a Video Greeting",
	"a21" => "Who Viewed Me       ",
	"51" => "My Membership",
	"52" => "Profile Management",
	"53" => "My Photos",
		"54" => "Click to manage photos",
	"55" => "Edit Profile",
		"56" => "Click to update profile info",
	"57" => "My Friends",
		"58" => "Click to view your friends",
	"59" => "Manage My Comments",
		"60" => "Click to view your comments",
	"61" => "My Photos",
		"62" => "Click to upload &amp; manage photos",
	"63" => "My Matches",
		"64" => "Click to edit match settings",
	"65" => "My Privacy Settings",
		"66" => "Click to edit privacy settings",
	"67" => "Record a Video",
		"68" => "Record a profile message",
	"69" => "Create Message",
		"70" => "Click to send a new message",
	"71" => "SMS/Text Settings ",
		"72" => "Click to view SMS settings",
	"73" => "My Email Alert Settings",
		"74" => "Click to edit email settings",
	"75" => "Blogging",
		"76" => "Click to view your groups",
	"77" => "My Groups",
		"78" => "Click to view your groups",
	"79" => "My Events",
		"80" => "Click to view your events",
	"81" => "My Classifieds",
		"82" => "Click to view your classified",
	"83" => "My Blogs",
		"84" => "Click to view your blogs",
	"86" => "Here are some useful links to help you get started managing your profile. Upload photos and more. Get started today creating your perfect profile.",
	"87" => "Every relationship starts with a first move. Go ahead, and start connecting today. Your next match maybe a message away.",
	"88" => "Got something on your mind? Create or Edit a blog. Starting a blog is a great way for potential matches to get to know you even better.",

//
	"89" => "Here are some useful links to help you get started managing your account. You can update your main photo,  update your match settings and more!",
	"90" => "Your Potential Local Matches",
	"91" => "Click on a members photo to view their profile.",
	
	"92" => "Hello",
	"93" => "Standout, enter a catchy headline!",

	"94" => "Change Photo",
);
$lang_register_page = array(

	"1" => "username is already in use",
	"2" => "email is already in use",
	"3" => "email address is invalid",
	"4" => "The password is too short!",
	"5" => "The username is too short!",
	"6" => "The username contains invalid characters!",
	"7" => "The password is too short!",
	"8" => "Please complete all of the fields",
	"9" => "The verification image was wrong!",
	"10" => "You must upload a photo",
	"11" => "Thank you, please check your email for your activation code.", 
	"a6" => "",	 
	"a8" => "",
	"a10" => "",
	"a11" => "Retype Password",
	"a12" => "",
	"a13" => "Required Account Details",	
	"a14" => "Upload a photo",
	"a15" => "Let's Add Some Photos",	
	"a16" => "Maximum size: 1Mb, minimum size: 105 x 105 pixels, format jpg.",	
	"a17" => "All members are required to have a photo. Go ahead and show us your smile!<br /><br/ > For best results we recommend you select a photo that highlights your face, you should be easily recognizable, alone, and facing the camera.",
	"a18" => "Web site Updates",
	"a19" => "Receive Promotional Offers",
	"a20" => "Receive Updates and Alerts",
	"a22" => "This is how we control spam!",
	"a23" => "I have read, understood and agree to abide by the",
	"a24" => "Terms and Conditions",
	"a25" => " of use",
	"a26" => "Complete Registration",		
	"28" => "Creating Your Account",
	"29" => "Please wait a few moments while we create your account",
	"30" => "Updating Your Account",
	"31" => "Please wait a few moments whilst we updated your account",
	"32" => "That's available! ",
	"33" => "Congratulations your account %s has been successfully created.",
	"34" => "We have sent an activation email to the following email address:",
	"35" => "All you need to do is retrieve the email from your account and click on the link in the message.",
	"36" => "If you have not received your activation email, please re-enter or change your email address below:",

);
$lang_account_page = array( 
	"5" => "Text Styles",
	"6" => "Background Colors",
	"7" => "Menu Bar and Header",
	"8" => "Box Colors",
	"9" => "Body Font",
	"10" => "Header Front",
	"11" => "Page Background",
	"12" => "Select Image",
	"13" => "File",
	"14" => "Display Type",
	"15" => "Delete Saved Image",
	"16" => "Image Box Background",
	"17" => "Header Box Background",
	"18" => "Header Box Text",
	"19" => "Main Text Color",
	"20" => "Box Header Text",
	"21" => "Box Top Background",
	"22" => "Box Body Background",
	"23" => "Don't Repeat Image",
	"24" => "Tile Image",
	"25" => "Repeat Vertically",
	"26" => "Repeat Horizontally",
	"27" => "You have no comments",
	"28" => "Delete My Video Greetings",
	"29" => "Use the options on the left to update the profile color scheme for your profile. You can customize the colours and upload your own background and header images.",
);
$lang_messages_page = array(

	"1" => "The username you entered doesn't exist, please check and try again.",
	"2" => "You cannot send messages to yourself",
	"3" => "The subject like is too short",
	"4" => "Sorry, you must upgrade to VIP!",
	"5" => "You are blocked from sending messages to this user",
	"a2" => "View My Inbox",
	"a3" => "Please wait while we send your messages",
	"a4" => "This may take a few minutes, thank you for your patience.",
	"a5" => "Enter members usernames below",
	"a6" => "",
	"a17" => "read",
	"a21" => "Sent to",
	"a33" => "View My Inbox",
	"a34" => "Message Sent By",
	"a35" => "at",	
	"a36" => "Sorry, it's empty!",
	"a38" => "select display limit",
);
$lang_gallery_page = array(

	"1" => "Photo Upload Failed",
	"2" => "Upload Completed",
	"3" => "You are at the max for this membership level.",	
	"4" => "The music file you upload is invalid or corrupt, please check and try again.",
	"5" => "The video file you upload is invalid or corrupt, please check and try again.",
	"6" => "the photo file type is invalid",
	"7" => "Success!",
	"8" => "Photo Album Created Successfully, Now upload some photos.",
	"9" => "Photos Updated Successfully",
	"10" => "file upload error",	
	"a1" => "My Photos",
	"a2" => "Upload Photos",
	"a4" => "Please wait while we save your file.",
	"a5" => "This may take a few minutes depending on the size of your file.",
	"a7" => "Select the album your file will be saved to.",
	"a15" => "Maximum size: 1Mb, minimum size: 105 x 105 pixels, Formats .jpg and .png",
	"a16" => "Recommendation: for best results we recommend you select a photo that highlights your face, you should be easily recognizable, alone, and facing the camera.",
	"a17" => "Display Photo",
	"a20" => "",	
	"a22" => "Maximum size: 3Mb, format mp3, midi and mpeg.",
	"a23" => "Recommendation: for best results we recommend you select a music file of less than 1Mb for fast uploading and listening.",
	"a24" => "Enter YouTube URL",
	"a25" => "Enter the Youtube web page address only, NOT the embed code.",
	"a26" => "Example: Copy the web site address such as http://youtube.com/watch?v=Dc541D7ny9I42234 into the box above to capture the youtube video.",
	"a28" => "Maximum size: 5Mb, format mpg, mpeg, mov, qt, mxu, movie and divx files.",
	"a29" => "Recommendation: for best results we recommend you select a video that you have recently created.",	
	"a32" => "Upload Photo",
	"a40" => "Upload Photos",
	"a44" => "display pic",
	"a46" => "you have no photos in this album",
	"a53" => "there are no comments for this photo",
	"a62" => "Upload Photos",
	"a64" => "Create a descriptive title for your new photo. eg. My Summer Holiday Photos",
	"a66" => "Private Photo Album",
	"a69" => "Adult Images",
	"a70" => "Public Photo Album",
	"a71" => "The album viability options allow you to select who can and who cannot view your album photos.",
	"a74" => "Enter a memorable title for your photo",	
	"76" => "Leave blank if you dont wish to password protect your photo album.",
);
$lang_match_page = array(

	"1" => "Has just create a new LoveQuiz, click here to take the test!",
	"2" => "LoveQuiz added successfully, now you can add some questions..",
	"3" => "LoveQuiz Description Updated",
	"4" => "The title is too short",
	"5" => "Question Added Successfully",
	"6" => "Question Updated Successfully.",
	"7" => "Thank You, your match results have been updated",
	"8" => "Match Settings Saved",					
	"a1" => "My Match Settings",
	"a3" => " Total Matches found for your current match settings!",
	"a4" => "Create New Match Settings",
	"a5" => "Search Match Results",
	"a8" => "Update My Match Settings",	 
	"a10" => "Enter a title for your LoveQuiz. For example: Let's test our Compatibility!",
	"a13" => "Manage LoveQuiz",	
	"a22" => "you have not create a LoveQuiz",  
	"a31" => "Please complete all five fields",	
	"a33" => "LoveQuiz Questions",
	"a36" => "Possible LoveQuiz Answers",
	"a37" => "Select your preferred answer by checking the box on the right next to the answer",
	"a42" => "You have not created any questions! Use the form above to create new questions.",
	"a48" => "you have no match results for this LoveQuiz",
	"a57" => "Take LoveQuiz",	
	"a60" => "LoveQuizzes I have taken",
	"a61" => "My LoveQuiz Results",	
	"a66" => "you have no match results for this LoveQuiz",	
	"a67" => "Update Settings",
	"10" => "Select LoveQuiz Icon",
	"11" => "Times Taken",

);
$lang_ajax = array(
 
	"2" => "Vote Added thank You",
	"3" => "You cannot send any more winks today!",
	"4" => "Your wink has been sent to",
	"5" => "Your wink has been sent",
	"6" => "We are a private community. Please Login or Join us today!",
	"7" => "Updated Successfully!",	
	"8" => "Photos Deleted Successfully",
	"9" => "Comment Approved Successfully",
	"10" => "Comment Delete Successfully",
	"11" => "Comments Sent for approval.",
	"12" => "Main Photo Updated",
	"13" => "Match Result Deleted",	
	"14" => "Question Deleted",
	"15" => "Match Test Deleted",
	"16" => "comment deleted",
	"17" => "comment approved",
	"18" => "Your comments have been sent for approval.",
	"19" => "Blog Entry Deleted",
	"24" => "Photo Deleted",
	"26" => "The username is too short!",
	"27" => "The username contains invalid characters",
	"28" => "The username is already taken, try an alternative",	
	"29" => "Invalid email address",
	"30" => "The password is too short!",
	"31" => "Must contain numbers and letters",	
	"32" => "sent you a Wink!",
	"33" => "You have received a wink from",	
	"34" => "This members is already on your list.",
	"35" => "please login",
	"37" => "messages per day",
	"38" => "winks per day",
	"39" => "Send up to",
	"40" => "Up to",
	"41" => "Highlighted profile in search results",
	"42" => "Featured profile on our home page",
	"43" => "Extra SMS Credits",
	"44" => "Please <a href='index.php?dll=login'>login</a> to post comments",
	"45" => "This email address is already in use",
	"46" => "Not so fast! We are a private community. Already a member? Login. If not, wait's the wait?",
	"47" => "Event details have been updated successfully.",
	"48" => "Event Deleted Successfully",			

);

// ORDER COMPLETE PAGE
$LANG_ORDER_MENU = array(
	"thankyou" => "Upgrade Successful",	
		"thankyou_?" => "Congratulations you are now a VIP Member. We would like to thank you for helping support our community. To ensure your account is fully updated, please logout of your account then log back in for all changes to take affect. We hope that you are our next success story. Good Luck!",
	"cancel" => "Upgrade Canceled",	
		"cancel_?" => "Your upgrade request was canceled. No money was taken from you at this time. If you have any questions please dont hesitate to contact us.",
	"error" => "Upgrade Error",	
		"error_?" => "An error occurred during your upgrade. Please contact us today if you have any problems with your account.",

);
$LANG_TOUR = array(
	"a1" => "The RBL Difference",
	"a3" => "Not yet a member? Join Here",	
	"a4" => "<p><strong>Leave the games behind. It's all real here.</strong><br />  We are an online dating community that caters to black singles who know exactly what they want. Join FREE today, and start meeting like-minded individuals who are looking for something real.</p><p> RealBlackLove.com is the only dating site that gives its members a full year of access after upgrade. We want to see you succeed.</p><p>Registration is super fast and you will be up and running in three minutes, join now to get started. <p>",	
	"a6" => "<p><strong>Wink system</strong><br />If your a bit shy when it comes to telling that potential special someone how you feel then we're here to help get the ball rolling.</p><p>Our advanced wink system will give you the ability to let others know your interested in them without saying to much!</p><p> </p>",
	"a7" => "<p><strong>Start searching</strong><br />Once registered use our powerful system to search for your perfect match. Its quick, easy to use and lets you search for what you think is important in a date. Whether its their appearance, religion, lifestyle or professional background, our system lets you make the calls.</p><p>You can look at as many profiles as you like, and can view quality photo albums from our great looking members.</p>",
	"a8" => "<p><strong>Our mission is real</strong> <br />We want to help black singles find relationships with long term potential and success. We pride ourselves in hosting the most relationship-focused black singles, so our members can be assured they are surrounded by individuals serious about their love life. We know what our members want, and why they choose our site to meet people.",
	"a9" => "<p><strong>SMS Alerts</strong><br><p>Our advanced SMS alert system keeps you up to date with messages and possible love interests so you stay connected!</p><p>Our built in SMS system means you can receive new wink and message alerts right to your mobile phone!</p><p>Don't wait around, get started today!</p>",
);
 
$_LANG_LOGIN = array(
	"login" => "Member Login",	
		"login_?" => "",
	"1" => "this account is suspended",
	"2" => "This account has not yet been activated, please check your email and enter the validate code.",
	"3" => "Account registration is successful! Note that your profile is kept on hold and will not be able to be viewed until approval by our member services team. Notification will be sent to your registered email address regarding your profile status upon review. We moderate each and every profile to ensure we maintain a safe community for our members. This is what makes RealBlackLove.com one of the most trusted dating websites online. We appreciate your patience.",
	"4" => "Your login details have failed. Please check and try again.",
	"5" => "Sorry but the code you entered is not correct.",
	"6" => "Your account has been created and an activation link has been sent to the e-mail address you entered. Note that you must activate the account by clicking on the activation link when you get the e-mail before you can login.<br><br><p>
If you haven't received the activation email, please check your spam folder. If the activation email isn't in there either, please <a href='index.php?dll=contact'>contact</a> us and we will activate your account manually.</a> ",
	"7" => "Your email address could not be found in our database.",
	"8" => "Your email address is invalid. Please check and try again.",
	"9" => "Your email address is invalid. Please check and try again.",
	"10" => "Thank You. We have sent a new password to your email.",
	"11" => "Your account is already activated.",	
	"a3" => "Username or Email Address",
	"a7" => "Enter Activation Code",
	"a11" => "Resend Activation code",
	"a12" => "Enter your email address",
	"a13" => "Activate Account",
	"a15" => "Activation Code",
	"a16" => "Resend Code",
);


$LANG_INVITE_PAGE = array(
 	"7" => "Your login details are incorrect, please check and try again.",
	"a28" => "We have found",
	"a29" => "contacts",
	"a30" => "contacts have already joined as members.",
	"a31" => "Would you like us to send an invitation to the others?",	
	"a32" => "Yes Please",
	"a33" => "No Thanks",
);
$LANG_GAMES_PAGE = array(
	"1" => "Last Played",
	"2" => "Played",
	"3" => "Times",
	"4" => "Top Player",
	"5" => "Top Score",
	"6" => "Leader Board",
	"7" => "2nd and 3rd",
	"8" => "Play Now",
 	"9" => "All Games",
);

$LANG_SETTINGS = array(
	 
	"5" => "Mobile numbers should start with a 0",
	"6" => "Mobile numbers is either invalid or inappropriate.",
	"7" => "SMS Details Updated",
	"9" => "Password Updated",
	"10" => "Your current password does not match",
	"11" => "Account marked for deletion",
	"12" => "Account reactivated",	
	"a1" => "My mobile phone (SMS) alerts",
	"a2" => "My Mobile Number",
	"a3" => "Note. Please do not enter your country code in this field. Only your mobile number.",
	"a5" => "Note. this should be sent to the country where you are currently using your mobile phone.",
	"a6" => "Receive Message Alerts",
	"a9" => "I want to receive an email when someone sends me a new message.",	
	"a10" => "Receive Wink Alerts",
	"a11" => "I want to receive an email when someone sends me a wink.",	
	"a12" => "You  have",
	"a13" => "SMS Credits Left!",
	"a14" => "Click here to purchase more credits",
	"a16" => "My email alerts",	
	"a17" => "Email Address",
	"a18" => "Note. Please ensure this is a valid email address.",
	"a19" => "Message Alert",
	"a20" => "I want to receive an email when someone sends me a new message.",
	"a21" => "Wink Alert",
	"a22" => "I want to receive an email when a member winks with me.",
	"a23" => "New Friend Request",
	"a24" => "I want to be alerted when a member wants to add me to her list of contacts",
	"a25" => "New Match Results",
	"a26" => "I want to receive the list of 'community singles'",	
	"a27" => "Promotional Offers",	
	"a28" => "I want to be informed of partner promotional offers",
	"a29" => "Promotional Offers",	
	"a30" => "I want to receive promotional from RealBlackLove.com.",
	"a34" => "Enable Chat",	
	"a37" => "I want to display the instant messenger button on my profile",
	"a38" => "Auto Approve Friends",
	"a39" => "If you want new friend requests to be automatically approved, select yes.",
	"a40" => "Auto Approve Comments",
	"a41" => "I want to be alerted when a new message is received or when one of my messages is read.",	
	"a51" => "If you decide that you don't need your account anymore, and you're sure you won't want your username in the future, you can simply close the account. Check the box below to confirm you would like to cancel your account. Your account will then be deleted by our admin within the next 7 days.",
	"a52" => "Are you sure?",	

	"a53" => "Profile Access (non friends)",
	"a54" => "Select which parts of your profile to BLOCK from a non-friend.",
	"a55" => "Profile Access (friends)",
	"a56" => "Select which parts of your profile to BLOCK from a friend.",
						
);

$LANG_AFFILIATE_PAGE = array(
	"1" => "Account Updated Successfully",
	"2" => "Your account has been suspended",
	"3" => "the username is invalid",					
	"4" => "the email address has already been registered",
	"5" => "the email address is invalid",
	"6" => "the password is too short",
	"7" => "the username address is invalid",
	"8" => "the username contains invalid characters",
	"9" => "the password is too short",
	"10" => "please complete all the fields",
	"11" => "Thank you, your account has now been created successfully, you can now login.",	
	"12" => "Your email address is invalid. Please check and try again.",
	"13" => "Your email address is invalid. Please check and try again.",
	"14" => "Thank You. We have sent a new password to your email.",
	"a1" => "RealBlackLove.com Partner",
	"a2" => "Join our Partner program",
	"a3" => "Already an Partner? Login Here",
	"a4" => "Partner Login Details",
	"a14" => "Zip/Post Code",
	"a20" => "Make check payable to",
	"a21" => "Not yet an Partner?",
	"a23" => "Username or Email Address",
	"a26" => "Enter your email address",	
	"a27" => "Transaction History",
	"a29" => "Total Earned",
	"a31" => "Paid",	
	"a32" => "Account Created",
	"a33" => "Current Commission Rate",
	"a34" => "Account Summary",
	"a35" => "Total Click-thru's",
	"a36" => "Total Registered",
	"a37" => "Total Subscriptions",
	"a38" => "Total Earned",
	"a39" => "Total Paid",
	"a40" => "Balance",
	"a41" => "Paypal email",
	"a42" => "Paypal is our trusted provider which we provide commission proceeds.",

);
$GLOBALS['_META'] = $LANG_;	
$GLOBALS['LANG_COMMON'] = $LANG_COMMON;
$GLOBALS['_LANG_ERROR'] = $LANG_ERROR;
$GLOBALS['_LANG'] = $LANG_BODY;
$GLOBALS['LANG_NETWORK']=  $LANG_INVITE_PAGE; 
$GLOBALS['LANG_GAMES'] = $LANG_GAMES_PAGE;
$GLOBALS['LANG_SETTINGS'] = $LANG_SETTINGS;
$GLOBALS['LANG_WELCOME'] = $LANG_WELCOME;
$GLOBALS['LANG_GLO_OPTIONS'] = $lang_global_options;
//$GLOBALS['LANG_LOGIN'] = $lang_main_sub_sub22;
$GLOBALS['LANG_ORDER'] = $lang_subscribe;
$GLOBALS['LANG_OVERVIEW'] = $lang_overview_page;
$GLOBALS['LANG_REGISTER'] = $lang_register_page; 
$GLOBALS['LANG_ACCOUNT'] = $lang_account_page;
$GLOBALS['LANG_MESSAGES'] = $lang_messages_page;
$GLOBALS['LANG_GALLERY'] = $lang_gallery_page;
$GLOBALS['LANG_MATCH'] = $lang_match_page;
$GLOBALS['LANG_AJAX'] = $lang_ajax;
?>