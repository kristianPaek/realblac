...
?>
	...
	<ul class="form"><div class="CapBody">        
		...
		<li><label><font color="#693333">Destination</font> </label><select name="aid">  <?=$gallery_display_showalbums ?> </select></li>
...
<div class="album-gallery">	  
  <ul>
	...
	<li id='Album_<?=$value['aid']?>'  style="background:white;color:#000000;">
		...
		<img src="<?=DB_DOMAIN ?>images/DEFAULT/_gal/add.gif"  align="absmiddle" style="border:0px;"> <a href="<?=DB_DOMAIN ?>index.php?dll=gallery&sub=upload"><?=$GLOBALS['_LANG']['_add'] ?></a> |
		<img src="<?=DB_DOMAIN ?>images/DEFAULT/_gal/delete.gif"  align="absmiddle" style="border:0px;"> <a href="#" onclick="Effect.Fade('Album_<?=$value['aid'] ?>'); DeleteAlbum('<?=$value['aid'] ?>'); return false;"><?=$GLOBALS['_LANG']['_delete'] ?></a>
		  ...
		  <ul>
			...
			<div id='File_<?=$value['id'] ?>'>		
				<li  style="background:white; height:150px;">
							...
							<div class="ClearAll"></div>
							<div class="PicFrame">
								<div class="PicFramebody"><a href="<?=$value['edit_link'] ?>"<? if($value['default'] ==1){ ?>class="defaultFile"<? } ?>><img src="<?=$value['image'] ?>"></a></div>
								
								<h4><a href="<?=DB_DOMAIN ?>index.php?dll=gallery&sub=upload&eid=<?=$value['id'] ?>">
<U>Edit Photo</U></a></h4></div>							
							...
							<small style="font-size:10px;">
								...
								[ <a href='#' onclick="Effect.Fade('File_<?=$value['id'] ?>'); DeleteFile('<?=$value['id'] ?>'); return false;"><?=$GLOBALS['_LANG']['_delete'] ?></a> ]
							</small>	
...
?>
	...
	<strong><?=$GLOBALS['_LANG']['_title'] ?>:</strong> <br><span id="FileCaption1" style="margin-bottom:15px;"> <?=$FileData['title'] ?></span>   <br>
	<strong><?=$GLOBALS['_LANG']['_description'] ?>:</strong> <br> <span id="FileDesc"> <?=$FileData['description'] ?></span>
	
	</div>
	<span><img src="<?=DB_DOMAIN ?>images/DEFAULT/_icons/new/pencil.png" width="16" height="16" align="absmiddle"> <a href="<?=DB_DOMAIN ?>index.php?dll=gallery&sub=upload&eid=<?=$item3_id ?>"><?=$GLOBALS['_LANG']['_edit'] ?></a></span><br>
	
	<br>
	<span><img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/emoticon_tongue.png" width="16" height="16" align="absmiddle"> (<?=$FileData['rating'] ?> %) <?=$FileData['rating_image'] ?> </span><br>
	<br>
	<img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/zoom.png" alt="views" align="absmiddle"> (<? print $FileData['views']; ?> <?=$GLOBALS['_LANG']['_views'] ?>) <br>
	<p>
	<span><?=$GLOBALS['_LANG']['_album'] ?>: </span><SPAN class=tags><? print $FileData['album_title']; ?></SPAN>&nbsp; <img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/wrench.png" align="absmiddle"><BR>
	</p>
	...
	<input name="do_page" type="hidden" value="gallery" class="hidden">
	<input name="sub" type="hidden" value="albums" class="hidden">
	<input name="aid" type="hidden" value="<?=$item2_id ?>" class="hidden">
	<input name="lid" type="hidden" value="<?=$item3_id ?>" class="hidden">
	<input type="hidden" name='do' value="changeAlbum" class="hidden">
	  <select name="new_aid" class="input">
		<?=$gallery_display_showalbums ?>
	  </select>
	  <input type="submit" name="Submit" value="<?=$GLOBALS['_LANG']['_save'] ?>" class="MainBtn">
	</form>
	<br>
	<span><img src="<?=DB_DOMAIN ?>images/DEFAULT/_icons/16/lock.gif" width="16" height="16" align="absmiddle"> <?=$GLOBALS['_LANG']['_adultContent'] ?>: <?=$FileData['adult_content']; ?></span>
