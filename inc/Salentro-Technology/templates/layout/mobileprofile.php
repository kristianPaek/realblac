...
<div id="messages"><div style="" class="message-good">
<img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/wrench.png" align="absmiddle"> <a href="<?=DB_DOMAIN ?>mobile.php?dll=mobileaccount&sub=edit&id=<?=$profileId ?>"> [ Edit Profile ] </a>
...
<div class="profile_box_body ">
 <table width="100%"  border="0">
  <tr>
    <td width="30%"><strong>Membername:</strong></td>
    <td width="70%"> <?=substr($profileUsername,0,30) ?> </td>
  </tr>
<? if($MyProfileGlobals['gender'] != 2710){ ?>
  <tr>
    <td><strong>My Age:</strong></td>
    <td> <?=$MyProfileGlobals['age'] ?> <?=$GLOBALS['_LANG']['_yold'] ?> <? if(D_STARSIGN ==1){ ?> (<?=$MyProfileGlobals['starsign'] ?>)<? } ?> <?=$MyProfileGlobals['MyGender'] ?> </td>
  </tr>
<? } ?>
  <tr>
    <td><strong>Reside in:</strong></td>
    <td> <?=$MyProfileGlobals['location'] ?>  </td>
  </tr>  <tr>
    <td colspan=2><p style="font-size:14px;">Last login was <?=showTimeSince($MyProfileGlobals['lastlogin']) ?></p></td>
  </tr></table></div>
...
<div id="ProfileOptionsBox">
	...
	<? if($show_page=="overview"){ ?>
	 ...
	 <p id="winkp">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a <? if($_SESSION['auth'] =="yes"){  if(is_array($PACKAGEACCESS[$_SESSION['packageid']]) && !in_array("wink-messages",$PACKAGEACCESS[$_SESSION['packageid']])){ ?> href="javascript:void(0)" onclick="sendwink('<?=$_SESSION['uid']?>','<?=$profileUsername ?>');" 
		
		
		<? }else{ ?> href="<?=DB_DOMAIN ?>mobile.php?dll=subscribe" <? } ?> <? }else{ ?> href="<?=DB_DOMAIN ?>mobile.php" <? } ?>  class="pLink"><font size="4" face="ARIAL">W I N K&nbsp;&nbsp;&nbsp;ME</FONT></a></p><hr>
        
        
        
		<p> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a <? if($_SESSION['auth'] =="yes"){  if(is_array($PACKAGEACCESS[$_SESSION['packageid']]) && !in_array("messages-create",$PACKAGEACCESS[$_SESSION['packageid']])){ ?> href="<?=DB_DOMAIN ?>mobile.php?dll=mobilemessages&sub=create&n=<?=$profileUsername ?>" <? }else{ ?> href="<?=DB_DOMAIN ?>mobile.php?dll=subscribe" <? } ?> <? }else{ ?> href="<?=DB_DOMAIN ?>mobile.php" <? } ?>  class="pLink"><font size="4" face="arial">M E S S A G E&nbsp;&nbsp;&nbsp;M E</FONT></a></p><hr>
		<? if(D_WINK ==222){ ?><p><a <? if($_SESSION['auth'] =="yes"){ if(is_array($PACKAGEACCESS[$_SESSION['packageid']]) && !in_array("chatroom-wink",$PACKAGEACCESS[$_SESSION['packageid']])){ ?> href="javascript:void(0);" onclick="openQuickWink(<?=$profileId ?>,'<?=$profileUsername ?>','<?=$MyProfileGlobals['image'] ?>'); return false;" <? }else{ ?> href="<?=DB_DOMAIN ?>mobile.php?dll=subscribe" <? } ?> <? }else{ ?> href="<?=DB_DOMAIN ?>mobile.php?dll=login" <? } ?>class="pLink"><?=$GLOBALS['LANG_COMMON'][10] ?></a></p><hr>
        </center>
<? } ?>
...
?>
		...
		<? if(D_HOTLIST ==1){ ?><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" class="pLink" onclick="ProfileAddNet(<?=$profileId ?>,1); alert('<?=$myalert ?>');return false;"><font size="4" FACE="arial">A D D&nbsp;&nbsp;&nbsp;F A V O R I T E</FONT></a></p><hr><? } ?>
		...
		<? if(D_RECOMMEND ==222){ ?> <p><img src="<?=DB_DOMAIN ?>images/DEFAULT/_icons/16/check.gif" width="16" height="16" align="absmiddle"> <a href="index.php?dll=recommend&pid=<?=$profileId ?>" class="pLink">Recommend to a friend</a></p><hr><? } ?>
...
<? } ?>
   ...
   <? } ?>
		...
		<h1 style="margin-top:5px;width:205px;"><?=$GLOBALS['LANG_GLO_OPTIONS']['1'] ?></h1><br>
		<p><img src="<?=DB_DOMAIN ?>images/DEFAULT/_acc/zoom.png" align="absmiddle"> <a href="<?=DB_DOMAIN ?>index.php?dll=profile&pId=<?=$profileId ?>"><?=$GLOBALS['LANG_COMMON'][11] ?> </a></p><hr>
	...
	<div class="profile_box_body">
	  ...
	  <center>
	<? foreach($RecentPhotos as $value){ ?><br /><img src="<?=$value['image'] ?>&x=280&y=400"  width="280" height="400"><br /><? } ?>	 
  </center>
	</div>
...
<? if(D_FRIENDS ==1){ ?>
	...
	*/
	...
	<p><br><a href="<?=DB_DOMAIN ?>mobile.php?dll=mobilesearch&friendid=<?=$profileId ?>"><?=$GLOBALS['_LANG']['_friendsList'] ?></a></p>
...
<? } ?>
	...
	*/
	...
	if(isset($profile1_data) && is_array($profile1_data) ){
 	if(GetProfileData($profileId,$value['groupid'],1)!=''){
 ...
 ?>
	...
	<div class="profile_box_title marginTop" id="DataBoxTitle<?=$value['groupid'] ?>">
		...
		<span class="goL">
		  ...
		  <span class="goR">
		     <? if($_SESSION['uid'] ==$profileId ){ ?><a href="<?=DB_DOMAIN ?>mobile.php?dll=mobileaccount&sub=edit" class="pLink"><?=$GLOBALS['_LANG']['_edit']?> </a> <? } ?>
		  </span>
		  <div class="ClearAll"></div>
...
<? } ?>
	...
	*/
	...
	<div class="profile_box_title marginTop">
		...
		<span class="goL">
			<h1><?=$GLOBALS['_LANG']['_quiz'] ?> <? if($_SESSION['uid'] ==$profileId ){ ?><a href="<?=DB_DOMAIN ?>index.php?dll=matches&sub=test" class="pLink"><?=$GLOBALS['_LANG']['_edit']?> </a> <? } ?> </h1>
		</span>
	...
	<a <? if($_SESSION['auth'] =="yes"){ ?>href="javascript:void(0);" onClick="NewpopUpWin('<?=DB_DOMAIN ?>inc/exe/quiz/start.php?item_id=<?=$profileId ?>&item2_id=<?=$value['id'] ?>', 450, 300);return false;" <? }else{ ?> href="<?=DB_DOMAIN ?>index.php?dll=login" <? } ?>  class="pLink">
	...
	*/
	...
	?>
		...
		<p><a href="index.php?dll=gallery&sub=search&fcid=<?=$profileId ?>">Click here to return to the album list.</a></p>
...
<? } ?>
	...
	<? if(!isset($BLOCKPAGEACCESS)){ ?>
		...
		<script type="text/javascript">
			...
			so.addVariable("loopSpeed", "1");
			so.addVariable("fadeSpeed", "1");
