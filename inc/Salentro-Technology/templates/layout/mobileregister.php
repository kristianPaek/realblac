...
<div id="reg_step_1" style="display:visible">
		...
		<div class="CapBody"> 
			...
			<li><label><?=$GLOBALS['_LANG']['_username'] ?></label><input name="username" type="text" class='input' id="regUsername" tabindex="1" style="width:260px;" onchange="validateUsername(this.value);" value="<? if(isset($_POST['username'])){print eMeetingOutput($_POST['username']); } ?>" size="35" maxlength="15"/> <span id="response_span"></span>
			<div class="tip"><?=$GLOBALS['LANG_REGISTER']['a6'] ?></div>
			</li>
	
			<li><label><?=$GLOBALS['_LANG']['_email'] ?></label><input type="text" class='input' size="35" style="width:260px;" name="email" id="regEmail" tabindex="2" onchange="validateEmail(this.value);" value="<? if(isset($_POST['email'])){print eMeetingOutput($_POST['email']); } ?>"/><span id="response_span_email"></span>
			<div class="tip"><?=$GLOBALS['LANG_REGISTER']['a8'] ?></div>
			</li>
			
			<li><label><?=$GLOBALS['_LANG']['_password'] ?> </label><input type="password" class='input' size="35" style="width:260px;" name="password" id="regPassword" tabindex="3" onchange="validatePassword(this.value);" value="<? if(isset($_POST['password'])){print eMeetingOutput($_POST['password']); } ?>" /><span id="response_span_pass"></span>
			<div class="tip"><?=$GLOBALS['LANG_REGISTER']['a10'] ?></div>
			</li>
	
			<li><label><?=$GLOBALS['LANG_REGISTER']['a11'] ?></label><input type="password" class='input' size="35" style="width:260px;" name="password_confirm" id="regRPassword" onChange="CheckPassword();" tabindex="4" value="<? if(isset($_POST['password_confirm'])){print eMeetingOutput($_POST['password_confirm']); } ?>" /><span id="response_span_rpass"></span>
...
<b>Your profile will be denied for failure to complete your registration thoroughly.</b><br /><br />By clicking the above button I agree to the terms and conditions of this website.</li>
			  </ul> 
			</div>
		</div>	
