...
<img src="/mobileheader.jpg"><br /><br />
...
<span id="response_message" class="responce_alert"></span>
	...
	<div id="Search">
		...
		<span class="a1">
			...
			<input name="sub" type="hidden" value="<?=$selected_page ?>" id="sub" class="hidden">Switch
...
<? if($value['attachment'] =="yes"){ ?><img src="<?=DB_DOMAIN ?>images/DEFAULT/_icons/new/photo.png" align="absmiddle"> <? }  ?>
		...
		<a href="<?=DB_DOMAIN ?>mobile.php?dll=mobilemessages&sub=read&msgid=<?=$value['id'] ?>" class="msgSub" style="text-decoration:none; font-size:11px;"> <? if(isset($_REQUEST['sub']) && $_REQUEST['sub'] !="sent"){ ?><?=$GLOBALS['_LANG']['_username'] ?>:<? }else{ ?><?=$GLOBALS['LANG_MESSAGES']['a21'] ?>: <? } ?> <?=$value['from'] ?> </a> 

		<a href="<?=DB_DOMAIN ?>mobile.php?dll=mobilemessages&sub=read&msgid=<?=$value['id'] ?>" class="msgSub"  style="text-decoration:none; font-size:11px;"><br> <?=$value['date'] ?> <?=$value['time'] ?> </a> 
