...
case "reset": {
						...
						while (!feof($file)) {
							...
							// check line and replace string
							  ...
							  if ( strstr($data[$counter], '"TMP_TXT_1"')  ) { 
						  	 
									$filecontent .= str_replace('"TMP_TXT_1","'.TMP_TXT_1.'"', '"TMP_TXT_1",""', $data[$counter]);
							  }	
								  elseif ( strstr($data[$counter], '"TMP_TXT_2","'.TMP_TXT_2.'"') ) {
							  	 
									$filecontent .= str_replace('"TMP_TXT_2","'.TMP_TXT_2.'"', '"TMP_TXT_2",""', $data[$counter]);
							  }	
							  elseif ( strstr($data[$counter], '"TMP_TXT_3","'.TMP_TXT_3.'"') ) {
							  	 
									$filecontent .= str_replace('"TMP_TXT_3","'.TMP_TXT_3.'"', '"TMP_TXT_3",""', $data[$counter]);
							  }	
							  elseif ( strstr($data[$counter], '"TMP_TXT_4","'.TMP_TXT_4.'"') ) {
							  	 
									$filecontent .= str_replace('"TMP_TXT_4","'.TMP_TXT_4.'"', '"TMP_TXT_4",""', $data[$counter]);
							  }	
							  elseif ( strstr($data[$counter], '"TMP_TXT_5","'.TMP_TXT_5.'"') ) {
							  	 
									$filecontent .= str_replace('"TMP_TXT_5","'.TMP_TXT_5.'"', '"TMP_TXT_5",""', $data[$counter]);
							  }	
							  elseif ( strstr($data[$counter], '"TMP_TXT_6","'.TMP_TXT_6.'"') ) {
							  	 
									$filecontent .= str_replace('"TMP_TXT_6","'.TMP_TXT_6.'"', '"TMP_TXT_6",""', $data[$counter]);
							  }			 
							  else{
									$filecontent .= $data[$counter];
							  }		 
				...
				$_POST['c7']=$_POST['pg'.$_POST['p1']];
						...
						while (!feof($file)) {
							...
							// check line and replace string
							 ...
							 // elseif ( strstr($data[$counter], "'TMP_PAGE_FOOTER','".TMP_PAGE_FOOTER."'")) {
							  	
									//$filecontent .= str_replace("'TMP_PAGE_FOOTER','".TMP_PAGE_FOOTER."'", "'TMP_PAGE_FOOTER','".$hidefooter."'", $data[$counter]);
							 // }	
...
$text = "";
$text = StripCR (trim(strip_tags(str_replace('"',"'",$_POST['txt1']))));
...
$text = "";
$text = StripCR (trim(strip_tags(str_replace('"',"'",$_POST['txt2']))));
...
$text = "";
$text = StripCR (trim(strip_tags(str_replace('"',"'",$_POST['txt3']))));
...
$text = "";
$text = StripCR (trim(strip_tags(str_replace('"',"'",$_POST['txt4']))));
...
$text = "";
$text = StripCR (trim(strip_tags(str_replace('"',"'",$_POST['txt5']))));
...
$text = "";
$text = StripCR (trim(strip_tags(str_replace('"',"'",$_POST['txt6']))));
		...
		*/						
		...
		case "create": {
				...
				$result = $DB->Insert("INSERT INTO `template_pages` (`name` ,`created` ,`content`)VALUES ( '".$_POST['name']."', NOW( ) , '".eMeetingInput($_POST['editor'])."')");				
		...
		case "update": {
			...
			if(!isset($pArray[1])){
								...
								if (get_magic_quotes_gpc()) {
								 ...
								 $DB->Update("UPDATE template_pages SET content='".eMeetingInput($bodytag)."' WHERE id='".$pArray[0]."'");
...
if(isset($Err) && !isset($_REQUEST['d'])){
	...
	if( isset($_POST['p']) || isset($RedirectPage) ){
		...
		if($page =="languages"){ $Err1 = $Err; $Err ="You <a href=".DB_DOMAIN."index.php?dll=logout>MUST LOGOUT</a> to see new language changes. ".$Err1; }
    ...
    while( $page = $DB->NextRow($result) )
    {
		if($meta){
			print "<option value='".$page['id']."**".$page['name']."'>".$page['name']."</option>";
						...
						print "<td><img src='inc/images/16x16/search.png' align='absmiddle'><a href='".DB_DOMAIN."index.php?dll=".$page['name']."' target='_blank'>".DB_DOMAIN."index.php?dll=".$page['name']."</a></td>";
...
function DisplayMenus($page="1"){
	...
	foreach($ThisArray as $key => $value){
		print '<tr>
			<td><input name="nkey'.$i.'" value="'.$key.'" type="text" class="mgrey" id="nkey'.$i.'"> =></td>
